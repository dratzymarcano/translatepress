...
class LRP_Check_Invalid_Text{
    ...
    public function get_existing_translations_without_invalid_text( $dictionary, $prepared_query, $strings_array, $language_code, $block_type ){
        if ( $this->is_invalid_data_error() ){
            ...
            if ( $count <= 1 ){
                ...
                $array1 = $lrp_query->get_existing_translations( array_slice( $strings_array, 0, $half ), $language_code, $block_type );
                $array2 = $lrp_query->get_existing_translations( array_slice( $strings_array, $half ), $language_code, $block_type );
    ...
    public function insert_translations_without_invalid_text( $new_strings, $language_code, $block_type ){
        if ( $this->is_invalid_data_error() ){
            ...
            if ( $count <= 1 ){
                ...
                $lrp_query->insert_strings( array_slice( $new_strings, 0, $half ), $language_code, $block_type );
                $lrp_query->insert_strings( array_slice( $new_strings, $half ), $language_code, $block_type );
                return;
            }
        }
    ...
    public function update_translations_without_invalid_text( $update_strings, $language_code, $block_type ){
        if ( $this->is_invalid_data_error() ){
            ...
            if ( $count <= 1 ){
                ...
                $lrp_query->update_strings( array_slice( $update_strings, 0, $half ), $language_code, $block_type );
                $lrp_query->update_strings( array_slice( $update_strings, $half ), $language_code, $block_type );
                return;
            }
        }
    ...
    public function is_invalid_data_error(){
        // Using $lrp_disable_invalid_data_detection as a sort of apply_filters to turn off this feature.
        // Not using proper WP filter to reduce page load time. This function is executed many times.
        global $wpdb, $lrp_disable_invalid_data_detection;
        if ( !empty($wpdb->last_error) && !isset( $lrp_disable_invalid_data_detection) ) {
