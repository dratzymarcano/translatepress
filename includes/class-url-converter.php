...
class LRP_Url_Converter {
    ...
    public function add_language_to_home_url( $url, $path, $orig_scheme, $blog_id ){
        ...
        //if this is not set then don't do anything as this is an exception/error and $LRP_LANGUAGE should always be set
        if( empty( $LRP_LANGUAGE ) )
            return $url;

        if ( isset( $this->settings['add-subdirectory-to-default-language'] ) && $this->settings['add-subdirectory-to-default-language'] == 'no' && $LRP_LANGUAGE == $this->settings['default-language'] ) {
            return $url;
        }

        if( apply_filters( 'lrp_add_language_to_home_url_check_for_admin', true, $url, $path ) &&
            ( is_customize_preview() || $this->is_admin_request()  || $this->is_sitemap_path( $path ) || $this->url_is_file( $path ) ) )
            return $url;

        $url_slug = $this->get_url_slug( $LRP_LANGUAGE );
        ...
        if ( lrp_force_slash_at_end_of_link( $this->settings ) ) {
        ...
        return apply_filters( 'lrp_home_url', $new_url, $abs_home, $LRP_LANGUAGE, $path, $url );
    }
    ...
    public function add_hreflang_to_head() {
        ...
        $original_url     = str_replace( '#LRPLINKPROCESSED', '', $this->get_url_for_language( $default_language ) );
        if ( apply_filters( 'lrp-exclude-hreflang', false, $original_url ) ) {
            return;
        }

        $languages = $this->settings['publish-languages'];
        if ( isset( $_GET['lrp-edit-translation'] ) && $_GET['lrp-edit-translation'] == 'preview' ) {
    ...
    public function replace_iso_2_with_iso_3_for_hreflang($hreflang, $language = null){
        ...
        $hreflang_iso_1 = apply_filters('lrp_add_hreflang_correct_iso_code', array(
            'bel' => 'be'
        ));
    ...
    public function change_lang_attr_in_html_tag( $output ){
        global $LRP_LANGUAGE;
        $tp_lang = str_replace('_formal', '', $LRP_LANGUAGE); // de-de-formal is not a valid lang attribute.
        $lang = get_bloginfo('language');
        if ( $lang && !empty($tp_lang) ) {
            if ( apply_filters( 'lrp_add_default_lang_tags', true ) ) {
                $output = str_replace( 'lang="' . $lang . '"', 'lang="' . str_replace( '_', '-', $tp_lang ) . '"', $output );
            }
            if ( apply_filters( 'lrp_add_regional_lang_tags', true ) ) {
    ...
    public function add_tp_language_lang_attribute(){
        global $LRP_LANGUAGE;
        $html ='<template id="tp-language" data-tp-language="'. esc_attr($LRP_LANGUAGE) . '"></template>';
     ...
     * @param $language
     * @param $url
     * @param $lrp_link_is_processed
     * @return array|string
     */
    public function check_if_url_is_valid_and_set_cache( $cache_key, $language, $url, $lrp_link_is_processed = '' ){
        $debug = false;
        global $LRP_LANGUAGE;

        if ( apply_filters( 'lrp_skip_url_for_language', false, $url ) ){
            return (string) $url;
        }

        $hash = hash( 'md4', (string) $language . (string) $url . (string) $lrp_link_is_processed . (string) $LRP_LANGUAGE );

        $cached_url = lrp_cache_get( $cache_key . $hash, 'lrp' );
        ...
        if ( $url_obj === false ){
            $url_obj = new \LinguaPress\Uri($url);
            wp_cache_set('url_obj_' . hash('md4', $url), $url_obj, 'lrp' );
        }

        $abs_home_url_obj = lrp_cache_get('url_obj_' . hash('md4',  $this->get_abs_home() ), 'lrp');

        if ( $abs_home_url_obj === false ){
            $abs_home_url_obj = new \LinguaPress\Uri( $this->get_abs_home() );
            wp_cache_set('url_obj_' . hash('md4', $this->get_abs_home()), $abs_home_url_obj, 'lrp' );
        }

        if ( $LRP_LANGUAGE == $this->settings['default-language'] ){
            $lrp_link_is_processed = '';
        }

        if ( $this->is_sitemap_path($url_obj->getPath()) ){
            lrp_bulk_debug( $debug, array( 'url' => $url, 'abort' => 'is file' ) );

            wp_cache_set($cache_key . $hash, $url . $lrp_link_is_processed, 'lrp');

            return $url . $lrp_link_is_processed; //abort for files
        }
        ...
        if ( !$url_obj->isSchemeless() && $url_obj->getScheme() != 'http' && $url_obj->getScheme() != 'https' ){
            lrp_bulk_debug($debug, array('url' => $url, 'abort' => "is different scheme ".$url_obj->getScheme()));

            wp_cache_set($cache_key . $hash, $url . $lrp_link_is_processed, 'lrp');

            return $url . $lrp_link_is_processed; // abort for non-http/https links
        }

        if ( $url_obj->isSchemeless() && !$url_obj->getPath() ){
            lrp_bulk_debug($debug, array('url' => $url, 'abort' => "is anchor or has get params"));

            wp_cache_set($cache_key . $hash, $url, 'lrp');
        ...
        if ( $url_obj->getHost() && $abs_home_url_obj->getHost() && $url_obj->getHost() != $abs_home_url_obj->getHost() ){
            lrp_bulk_debug($debug, array('url' => $url, 'abort' => "is external url "));
        ...
        if ( $this->get_lang_from_url_string($url) === null && $this->settings['default-language'] === $language && $this->settings['add-subdirectory-to-default-language'] !== 'yes' ){
            lrp_bulk_debug($debug, array('url' => $url, 'abort' => "URL already has the correct language added to it and default language has subdir"));
        ...
        if ( strpos( $url, '/wp-json' ) !== false || strpos( $url, '/wp-admin' ) !== false ) {
            lrp_bulk_debug($debug, array('url' => $url, 'abort' => 'is wp-json or admin link'));

            wp_cache_set($cache_key . $hash, $url . $lrp_link_is_processed, 'lrp');

            return $url . $lrp_link_is_processed; // abort for wp-json or admin links
        }
     ...
     * @param string $lrp_link_is_processed
     * @return string
     */

    public function get_url_for_language ( $language = null, $url = null, $lrp_link_is_processed = '#LRPLINKPROCESSED') {
        $debug = false;
        // initializations
        global $LRP_LANGUAGE;
        ...
        $url = apply_filters( 'lrp_pre_get_url_for_language', $url, $language, $this->get_abs_home(), $this->get_lang_from_url_string( $url ), $this->get_url_slug( $language ) );

        $cached_url = $this->check_if_url_is_valid_and_set_cache('get_url_for_language_', $language, $url, $lrp_link_is_processed );
        ...
        if ( $lang_from_url_string === null) {
            ...
            lrp_bulk_debug($debug, array('url' => $url, 'abort' => "URL already has the correct language added to it"));
        } else {
        ...
        if( class_exists( 'WooCommerce' ) && !class_exists( 'LRP_IN_Seo_Pack' ) && $lang_from_url_string !== $language ){
            ...
            foreach ($english_woocommerce_slugs as $english_woocommerce_slug){
                ...
                $current_slug = lrp_get_transient( 'tp_'.$english_woocommerce_slug.'_'. $this->settings['default-language'] );
                if( $current_slug === false ){
                    $current_slug = lrp_x( $english_woocommerce_slug, 'slug', 'woocommerce', $this->settings['default-language'] );
                ...
                if( $this->lrp_get_woocommerce_saved_permalink($english_woocommerce_slug) === $current_slug ) {
                    if (strpos($new_url, '/' . $current_slug . '/') === false) {
                        $current_slug = lrp_get_transient('tp_' . $english_woocommerce_slug . '_' . $LRP_LANGUAGE);
                        if ($current_slug === false) {
                            $current_slug = lrp_x($english_woocommerce_slug, 'slug', 'woocommerce', $LRP_LANGUAGE);
                            set_transient('tp_' . $english_woocommerce_slug . '_' . $LRP_LANGUAGE, $current_slug, 12 * HOUR_IN_SECONDS);
                        }
                    }
                    $translated_slug = lrp_get_transient('tp_' . $english_woocommerce_slug . '_' . $language);
                    if ($translated_slug === false) {
                        $translated_slug = lrp_x($english_woocommerce_slug, 'slug', 'woocommerce', $language);
        ...
        $new_url = apply_filters( 'lrp_get_url_for_language', $new_url, $url, $language, $this->get_abs_home(), $lang_from_url_string, $this->get_url_slug( $language ) );
        wp_cache_set('get_url_for_language_' . $hash, $new_url . $lrp_link_is_processed, 'lrp');
        return $new_url . $lrp_link_is_processed;
    }
    ...
    public function url_is_file( $url = null ){
        ...
        if ( empty( $url ) || $translation_render->is_external_link($url, $home_url ) ){ // Use unfiltered home_url due to infinite loop
            ...
            if ( strpos( $url, 'wp-content/uploads' ) !== false ) {
                ...
                if(apply_filters('lrp_is_file', true, $path)) {
                    $return = @is_file( $path );
                }else{
                    $return = true;
                }
        ...
        return apply_filters( 'lrp_url_is_file', $return, $url, $this->get_abs_home() );
    }

    public function does_url_contains_array($return, $path){
        $elements_to_avoid = apply_filters( 'lrp_elements_to_avoid_when_is_file_is_called', array("index.php", "/../"));
    ...
    public function get_abs_home() {
        $this->absolute_home = lrp_cache_get('get_abs_home', 'lrp');
        ...
        if ( apply_filters('lrp_adjust_absolute_home_https_based_on_server_variable', true) ) {
        ...
        $this->absolute_home = apply_filters('lrp_filter_absolute_home_result', $this->absolute_home);

        wp_cache_set( 'get_abs_home', $this->absolute_home, 'lrp' );
    ...
    public function get_lang_from_url_string( $url = null ) {
        ...
        $language = lrp_cache_get('url_language_' . hash('md4', $url) , 'lrp' );
        if ( $language !== false ){
            return $language;
        }
        ...
        if( $url_obj->getPath() ){
            ...
            $possible_path = lrp_remove_prefix($abs_home, $url_obj->getPath());

            $lang = ltrim( $possible_path,'/' );
            $lang = explode('/', $lang);
            if( $lang == false ){
                wp_cache_set('url_language_' . hash('md4', $url), null, 'lrp');
                return null;
            }
            // If we have a language in the URL, the first element of the array should be it.
            $lang = $lang[0];

            $lang = apply_filters( 'lrp_get_lang_from_url_string', $lang, $url );
            ...
            if( isset($this->settings['url-slugs']) && in_array($lang, $this->settings['url-slugs']) ){
                ...
                if ( in_array( $language, $this->settings['publish-languages'] ) ||
                    ( in_array( $language, $this->settings['translation-languages'] ) && current_user_can(apply_filters( 'lrp_translating_capability', 'manage_options' )) ) ) {
                    wp_cache_set( 'url_language_' . hash( 'md4', $url ), $language, 'lrp' );
                    return $language;
                }
            }
        }
     ...
     * plugins_loaded priority 1. Basically before \LRP_IN_SP_Slug_Manager::translate_request_uri()
    ...
    public function cur_page_url( $translated_slugs = true ) {
        ...
        $req_uri          = lrp_cache_get( 'cur_page_url' . $translated_slugs, 'lrp' );
        ...
        if ( function_exists('apply_filters') ) $req_uri = apply_filters('lrp_curpageurl', $req_uri);

        wp_cache_set( 'cur_page_url' . $translated_slugs, $req_uri, 'lrp' );
    ...
    public function woocommerce_filter_permalinks_on_other_languages( $rewrite_rules ){
        ...
        if ( class_exists( 'WooCommerce' ) && !class_exists( 'LRP_IN_Seo_Pack' ) ) {
            global $LRP_LANGUAGE;
            ...
            if( $LRP_LANGUAGE != $this->settings['default-language'] ){
                ...
                if( empty($default_language_wc_permalink_structure) ) {
                    $default_language_wc_permalink_structure = lrp_get_transient( 'tp_default_language_wc_permalink_structure_'.$this->settings['default-language'] );
                    if( $default_language_wc_permalink_structure === false ) {
                        $default_language_wc_permalink_structure = array();
                        $default_language_wc_permalink_structure['product_rewrite_slug'] = lrp_x('product', 'slug', 'woocommerce', $this->settings['default-language']);
                        $default_language_wc_permalink_structure['category_rewrite_slug'] = lrp_x('product-category', 'slug', 'woocommerce', $this->settings['default-language']);
                        $default_language_wc_permalink_structure['tag_rewrite_slug'] = lrp_x('product-tag', 'slug', 'woocommerce', $this->settings['default-language']);
                ...
                $current_language_permalink_structure = lrp_get_transient( 'tp_current_language_wc_permalink_structure_'.$LRP_LANGUAGE );
                if( $current_language_permalink_structure === false ) {
                    ...
                    $current_language_permalink_structure['product_rewrite_slug'] = lrp_x('product', 'slug', 'woocommerce', $LRP_LANGUAGE);
                    $current_language_permalink_structure['category_rewrite_slug'] = lrp_x('product-category', 'slug', 'woocommerce', $LRP_LANGUAGE);
                    $current_language_permalink_structure['tag_rewrite_slug'] = lrp_x('product-tag', 'slug', 'woocommerce', $LRP_LANGUAGE);

                    set_transient( 'tp_current_language_wc_permalink_structure_'.$LRP_LANGUAGE, $current_language_permalink_structure, 12 * HOUR_IN_SECONDS );
                }
    ...
    public function woocommerce_filter_permalink_option( $value ){
        $lrp     = LRP_Lingua_Press::get_lrp_instance();
        $upgrade = $lrp->get_component( 'upgrade' );

        if ( class_exists( 'LRP_IN_Seo_Pack' ) && $upgrade->is_seo_pack_minimum_version_met() && ( !isset( $this->settings['lrp_advanced_settings']['load_legacy_seo_pack'] ) || $this->settings['lrp_advanced_settings']['load_legacy_seo_pack'] === 'no' ) ) {
            return $value;
        }

        // Only when SEO Pack add-on is disabled. Otherwise, SEO Pack uses a different system to handle this
        global $LRP_LANGUAGE, $lrp_wc_permalinks;

        //keep the unfiltered value in a global, we might need it later
        if( !isset( $lrp_wc_permalinks ) )
            $lrp_wc_permalinks = $value;

        if( $LRP_LANGUAGE != $this->settings['default-language'] ) {
            if( trim($value['product_base'], '/') === lrp_x( 'product', 'slug', 'woocommerce', $this->settings['default-language'] ) ){
                $value['product_base'] = '';
                ...
                if( wp_doing_ajax() ){
                    $value['product_base'] = lrp_x( 'product', 'slug', 'woocommerce', $LRP_LANGUAGE );
                }
            }else{
                ...
                if ( substr( $value['product_base'], 0, strlen('/product/' ) ) === '/product/' ) {
                    $value['product_base'] = substr_replace( $value['product_base'], '/' . lrp_x( 'product', 'slug', 'woocommerce', $LRP_LANGUAGE ) . '/', 0, strlen('/product/' ) );
                }
            }

            if( trim($value['category_base'], '/') === lrp_x( 'product-category', 'slug', 'woocommerce', $this->settings['default-language'] ) ){
                $value['category_base'] = '';
            }

            if( trim($value['tag_base'], '/') === lrp_x( 'product-tag', 'slug', 'woocommerce', $this->settings['default-language'] ) ){
                $value['tag_base'] = '';
            }
    ...
    public function prevent_permalink_update_on_other_languages( $value, $old_value ){
        ...
        if( apply_filters( 'lrp_keep_permalinks_unchanged', false ) ||
            ( isset($LRP_LANGUAGE) && $LRP_LANGUAGE != $this->settings['default-language'] && apply_filters( 'lrp_prevent_permalink_update_on_other_languages', true ) ) )
        {
            $value = $old_value;
        }
     ...
     */
    function lrp_get_woocommerce_saved_permalink( $english_woocommerce_slug ){
