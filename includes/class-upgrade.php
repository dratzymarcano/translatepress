...
class LRP_Upgrade {
	...
	public function register_menu_page(){
		add_submenu_page( 'LRPHidden', 'LinguaPress Remove Duplicate Rows', 'LRPHidden', apply_filters( 'lrp_settings_capability', 'manage_options' ), 'lrp_remove_duplicate_rows', array($this, 'lrp_remove_duplicate_rows') );
		add_submenu_page( 'LRPHidden', 'LinguaPress Update Database', 'LRPHidden', apply_filters( 'lrp_settings_capability', 'manage_options' ), 'lrp_update_database', array( $this, 'lrp_update_database_page' ) );
	}
	...
	public function check_for_necessary_updates(){
		...
		$stored_database_version = get_option('lrp_plugin_version');
		if( empty($stored_database_version) ){
            ...
            if ( version_compare( $stored_database_version, '1.3.0', '<=' ) ) {
                $this->lrp_query->check_for_block_type_column();
                $this->check_if_gettext_tables_exist();
            }
            ...
            if ( version_compare( $stored_database_version, '1.6.5', '<=' ) ) {
                $this->lrp_query->check_for_original_id_column();
                $this->lrp_query->check_original_table();
                $this->lrp_query->check_original_meta_table();
            }
			...
			if ( version_compare( $stored_database_version, '2.3.7', '<=' ) ) {
                $gettext_normalization = $this->lrp_query->get_query_component('gettext_normalization');
                $gettext_normalization->check_for_gettext_original_id_column();

                $gettext_table_creation = $this->lrp_query->get_query_component('gettext_table_creation');
        ...
        if( version_compare( LRP_PLUGIN_VERSION, $stored_database_version, '!=' ) ){
            update_option( 'lrp_plugin_version', LRP_PLUGIN_VERSION );
		}
	}
    ...
    public function migrate_auto_translate_slug_to_automatic_translation(){
        ...
        if( !isset( $mt_settings_option['automatically-translate-slug'] ) ){
            ...
            update_option( 'lrp_machine_translation_settings', $mt_settings_option );
        }
    }
    ...
    public function get_updates_processing_message( $key ){
        $messages = [
            'remove_cdata_original_and_dictionary_rows'     => __('Removing cdata dictionary strings for language %s...', 'linguapress' ),
            'remove_untranslated_links_dictionary_rows'     => __('Removing untranslated dictionary links for language %s...', 'linguapress' ),
            'remove_duplicate_gettext_rows'                 => __('Removing duplicated gettext strings for language %s...', 'linguapress' ),
            'remove_duplicate_dictionary_rows'              => __('Removing duplicated dictionary strings for language %s...', 'linguapress' ),
            'remove_duplicate_untranslated_dictionary_rows' => __('Removing untranslated dictionary strings where translation is available for language %s...', 'linguapress' ),
            'original_id_insert_166'                        => __('Inserting original strings for language %s...', 'linguapress' ),
            'original_id_cleanup_166'                       => __('Cleaning original strings table for language %s...', 'linguapress' ),
            'original_id_update_166'                        => __('Updating original string ids for language %s...', 'linguapress' ),
            'regenerate_original_meta'                      => __('Regenerating original meta table for language %s...', 'linguapress' ),
            'clean_original_meta'                           => __('Cleaning original meta table for language %s...', 'linguapress' ),
            'replace_original_id_null'                      => __('Replacing original id NULL with value for language %s...', 'linguapress' ),
            'gettext_original_id_insert'                    => __('Inserting gettext original strings for language %s...', 'linguapress' ),
            'gettext_original_id_cleanup'                   => __('Cleaning gettext original strings table for language %s...', 'linguapress' ),
            'gettext_original_id_update'                    => __('Updating gettext original string ids for language %s...', 'linguapress' ),
            'migrate_old_slugs_to_the_new_translate_table_structure_post_type_and_tax_284' => __( 'Migrating taxonomy and post type base slugs to new table structure...', 'linguapress' ),
            'migrate_old_slugs_to_the_new_translate_table_structure_post_meta_284'         => __( 'Migrating post slugs to new table structure for language %s...', 'linguapress' ),
            'migrate_old_slugs_to_the_new_translate_table_structure_term_meta_284'         => __( 'Migrating term slugs to new table structure for language %s...', 'linguapress' ),
            'show_error_db_message'                         => __( 'Finishing up...', 'linguapress' )
        ];
    ...
    public function get_updates_details(){
        return apply_filters( 'lrp_updates_details',
            array(
                'remove_cdata_original_and_dictionary_rows' => array(
                    'version'           => '0',
                    'option_name'       => 'lrp_remove_cdata_original_and_dictionary_rows',
                    'callback'          => array( $this->lrp_query,'remove_cdata_in_original_and_dictionary_tables'),
                    'batch_size'        => 1000,
                    'message_initial'   => '',
                ),
                'remove_untranslated_links_dictionary_rows' => array(
                    'version'           => '0',
                    'option_name'       => 'lrp_remove_untranslated_links_dictionary_rows',
                    'callback'          => array( $this->lrp_query,'remove_untranslated_links_in_dictionary_table'),
                    'batch_size'        => 10000,
                    'message_initial'   => '',
                ),
				'full_trim_originals_140' => array(
					'version'           => '1.4.0',
					'option_name'       => 'lrp_updated_database_full_trim_originals_140',
					'callback'          => array( $this, 'lrp_updated_database_full_trim_originals_140' ),
					'batch_size'        => 200
				),
				'gettext_empty_rows_145' => array(
					'version'           => '1.4.5',
					'option_name'       => 'lrp_updated_database_gettext_empty_rows_145',
					'callback'          => array( $this,'lrp_updated_database_gettext_empty_rows_145'),
					'batch_size'        => 20000
				),
                'remove_duplicate_gettext_rows' => array(
                    'version'           => '0',
                    'option_name'       => 'lrp_remove_duplicate_gettext_rows',
                    'callback'          => array( $this->lrp_query,'remove_duplicate_rows_in_gettext_table'),
                    'batch_size'        => 5000,
                    'message_initial'   => '',
                ),
                'remove_duplicate_dictionary_rows' => array(
                    'version'           => '0',
                    'option_name'       => 'lrp_remove_duplicate_dictionary_rows',
                    'callback'          => array( $this->lrp_query,'remove_duplicate_rows_in_dictionary_table'),
                    'batch_size'        => 1000,
                    'message_initial'   => '',
                ),
                'original_id_insert_166' => array(
                    'version'           => '1.6.6',
                    'option_name'       => 'lrp_updated_database_original_id_insert_166',
                    'callback'          => array( $this,'lrp_updated_database_original_id_insert_166'),
                    'batch_size'        => 1000,
                ),
                'original_id_cleanup_166' => array(
                    'version'           => '1.6.6',
                    'option_name'       => 'lrp_updated_database_original_id_cleanup_166',
                    'callback'          => array( $this,'lrp_updated_database_original_id_cleanup_166'),
                ...
                'original_id_update_166' => array(
                    'version'           => '1.6.6',
                    'option_name'       => 'lrp_updated_database_original_id_update_166',
                    'callback'          => array( $this,'lrp_updated_database_original_id_update_166'),
                    'batch_size'        => 5000,
                    'message_initial'   => '',
                ),
                'regenerate_original_meta' => array(
                    'version'           => '0', // independent of tp version, available only on demand
                    'option_name'       => 'lrp_regenerate_original_meta_table',
                    'callback'          => array( $this,'lrp_regenerate_original_meta_table'),
                    'batch_size'        => 200,
                    'message_initial'   => '',
                ),
                'clean_original_meta' => array(
                    'version'           => '0', // independent of tp version, available only on demand
                    'option_name'       => 'lrp_clean_original_meta_table',
                    'callback'          => array( $this,'lrp_clean_original_meta_table'),
                    'batch_size'        => 20000,
                    'message_initial'   => '',
                ),
                'replace_original_id_null' => array(
                    'version'           => '0', // independent of tp version, available only on demand
                    'option_name'       => 'lrp_replace_original_id_null',
                    'callback'          => array( $this,'lrp_replace_original_id_null'),
                    'batch_size'        => 50,
                    'message_initial'   => '',
                ),
                'gettext_original_id_insert' => array(
                    'version'           => '2.3.8',
                    'option_name'       => 'lrp_updated_database_gettext_original_id_insert',
                    'callback'          => array( $this,'lrp_updated_database_gettext_original_id_insert'),
                    'batch_size'        => 1000,
                ),
                'gettext_original_id_cleanup' => array(
                    'version'           => '2.3.8',
                    'option_name'       => 'lrp_updated_database_gettext_original_id_cleanup',
                    'callback'          => array( $this,'lrp_updated_database_gettext_original_id_cleanup'),
                    'progress_message'  => 'clean',
                    'batch_size'        => 1000,
                    'message_initial'   => '',
                ),
                'gettext_original_id_update' => array(
                    'version'           => '2.3.8',
                    'option_name'       => 'lrp_updated_database_gettext_original_id_update',
                    'callback'          => array( $this,'lrp_updated_database_gettext_original_id_update'),
                    'batch_size'        => 5000,
                    'message_initial'   => '',
                ),
                'migrate_old_slugs_to_the_new_translate_table_structure_post_type_and_tax_284' => array(
                    'version'            => '0',
                    'option_name'        => 'lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_post_type_and_tax_284',
                    'callback'           => array( $this, 'lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_post_type_and_tax_284' ),
                    'batch_size'         => 1000000,
                    'message_initial'    => '',
                ...
                'migrate_old_slugs_to_the_new_translate_table_structure_post_meta_284'         => array(
                    'version'            => '0',
                    'option_name'        => 'lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_post_meta_284',
                    'callback'           => array( $this, 'lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_post_meta_284' ),
                    'batch_size'         => 500,
                    'message_initial'    => '',
                ),
                'migrate_old_slugs_to_the_new_translate_table_structure_term_meta_284'         => array(
                    'version'            => '0',
                    'option_name'        => 'lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_term_meta_284',
                    'callback'           => array( $this, 'lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_term_meta_284' ),
                    'batch_size'         => 500,
                    'message_initial'    => '',
                ),
                ...
                'show_error_db_message' => array(
                    'version'            => '0', // independent of tp version, available only on demand
                    'option_name'        => 'lrp_show_error_db_message',
                    'callback'           => array( $this, 'lrp_successfully_run_database_optimization' ),
                    'batch_size'         => 10,
                    'message_initial'    => '',
                    'execute_only_once'  => true
                )
    ...
    public function lrp_successfully_run_database_optimization($language_code= null, $inferior_size = null, $batch_size = null){
    ...
    public function show_admin_error_message(){
        ...
        if ( $option_db_error_message === 'no' ) {
            add_action( 'admin_notices', array( $this, 'lrp_admin_notice_error_database' ) );
        }

    }

    public function lrp_admin_notice_error_database(){

        echo '<div class="notice notice-error is-dismissible">
            <p>' . wp_kses( sprintf( __('Database optimization did not complete successfully. We recommend restoring the original database or <a href="%s" >trying again.</a>', 'linguapress'), admin_url('admin.php?page=lrp_update_database') ), array('a' => array( 'href' => array() ) ) ) .'</p>
        </div>';

    }

	public function lrp_update_database_page(){
		require_once LRP_PLUGIN_DIR . 'partials/lrp-update-database.php';
	}

	/**
	 * Call all functions to update database
	 *
	 * hooked to wp_ajax_lrp_update_database
	 */
	public function lrp_update_database(){
		if ( ! current_user_can( apply_filters('lrp_update_database_capability', 'manage_options') ) ){
			$this->stop_and_print_error( __('Update aborted! Your user account doesn\'t have the capability to perform database updates.', 'linguapress' ) );
		}

		$nonce = isset( $_REQUEST['lrp_updb_nonce'] ) ? wp_verify_nonce( sanitize_text_field( $_REQUEST['lrp_updb_nonce'] ), 'tpupdatedatabase' ) : false;
		if ( $nonce === false ){
			$this->stop_and_print_error( __('Update aborted! Invalid nonce.', 'linguapress' ) );
		}
        ...
        if (isset($_REQUEST['initiate_update']) && $_REQUEST['initiate_update']=== "true" ){
            update_option('lrp_show_error_db_message', 'no');
        }
		if ( empty ( $_REQUEST['lrp_updb_action'] ) ){
			foreach( $updates_needed as $update_action_key => $update ) {
				$option = get_option( $update['option_name'], 'is not set' );
				if ( $option === 'no' ) {
					$_REQUEST['lrp_updb_action'] = $update_action_key;
					break;
				}
			}
			if ( empty ( $_REQUEST['lrp_updb_action'] ) ){
				$back_to_settings_button = '<a class="lrp-submit-btn button" href="' . site_url('wp-admin/options-general.php?page=lingua-press') . '">' . esc_html__('Back to LinguaPress Settings', 'linguapress' ) . '</a>';
				// finished successfully
				echo json_encode( array(
					'lrp_update_completed' => 'yes',
					'progress_message'  => '<p><strong>' . __('Successfully updated database!', 'linguapress' ) . '</strong></p>' . $back_to_settings_button
				));
				wp_die();
			}else{
				$_REQUEST['lrp_updb_lang'] = $this->settings['translation-languages'][0];
				$_REQUEST['lrp_updb_batch'] = 0;
                $updb_action = sanitize_text_field( $_REQUEST['lrp_updb_action'] );

                $update_message_initial = isset( $updates_needed[ $updb_action ]['message_initial'] ) ?
                                            $updates_needed[ $updb_action ]['message_initial']
                                            : __('Updating database to version %s+', 'linguapress' );

                $update_message_processing = $this->get_updates_processing_message( $updb_action ) ?
                                             $this->get_updates_processing_message( $updb_action ) :
                                             __('Processing table for language %s...', 'linguapress' );
                ...
                $request['progress_message'] .= '<br>' . sprintf( $update_message_processing, sanitize_text_field( $_REQUEST['lrp_updb_lang'] ) );//phpcs:ignore
			}
		}else{
			if ( !isset( $updates_needed[ $_REQUEST['lrp_updb_action'] ] ) ){
				$this->stop_and_print_error( __('Update aborted! Incorrect action.', 'linguapress' ) );
			}
			if ( !in_array( $_REQUEST['lrp_updb_lang'], $this->settings['translation-languages'] ) ) {//phpcs:ignore
				$this->stop_and_print_error( __('Update aborted! Incorrect language code.', 'linguapress' ) );
			}
		}

		$request['lrp_updb_action'] = sanitize_text_field( $_REQUEST['lrp_updb_action'] );
		if ( !empty( $_REQUEST['lrp_updb_batch'] ) && (int) $_REQUEST['lrp_updb_batch'] > 0 ) {
			$get_batch = (int)$_REQUEST['lrp_updb_batch'];
		}else{
			$get_batch = 0;
		}

        $extra_params = isset( $_REQUEST['lrp_updb_extra_params'] ) ? json_decode(base64_decode(sanitize_text_field($_REQUEST['lrp_updb_extra_params'] )), true) : array();
        if (!is_array($extra_params)) {
            $extra_params = array();
        }

		$request['lrp_updb_batch'] = 0;
		$update_details = $updates_needed[ sanitize_text_field( $_REQUEST['lrp_updb_action'] )];
		$batch_size = apply_filters( 'lrp_updb_batch_size', $update_details['batch_size'], sanitize_text_field( $_REQUEST['lrp_updb_action'] ), $update_details );
		$language_code = isset( $_REQUEST['lrp_updb_lang'] ) ? sanitize_text_field( $_REQUEST['lrp_updb_lang'] ) : '';
        ...
        if ( !$finalized_with_language ) {
			$request['lrp_updb_batch'] = $get_batch;
            $request['lrp_updb_extra_params'] = isset($callback_return['extra_params']) ? $callback_return['extra_params'] : array();
		}
		...
		if ( $finalized_with_language ) {
            ...
            if ( isset ( $this->settings['translation-languages'][ $index + 1 ] ) && (!isset($update_details['execute_only_once']) || $update_details['execute_only_once'] == false)) {
                ...
                $request['lrp_updb_lang']    = $this->settings['translation-languages'][ $index + 1 ];
                $request['progress_message'] .= __( ' done.', 'linguapress' ) . '</br>';
                $update_message_processing   = $this->get_updates_processing_message( sanitize_text_field( $_REQUEST['lrp_updb_action'] ) ) ?
                    $this->get_updates_processing_message( sanitize_text_field( $_REQUEST['lrp_updb_action'] ) )
                    : __( 'Processing table for language %s...', 'linguapress' );
                $request['progress_message'] .= '</br>' . sprintf( $update_message_processing, $request['lrp_updb_lang'] );

                    } else {
                        // finish action due to completing all the translation languages
                        $request['progress_message'] .= __( ' done.', 'linguapress' ) . '</br>';
                        $request['lrp_updb_lang']    = '';
                        ...
                        $request['lrp_updb_action'] = '';
                    }
		}else{
			$request['lrp_updb_lang'] = $language_code;
            $request['progress_message'] .= '.';
		}
		...
		$query_arguments = array(
			...
			'lrp_updb_action'           => $request['lrp_updb_action'],
			'lrp_updb_lang'             => $request['lrp_updb_lang'],
			'lrp_updb_batch'            => $request['lrp_updb_batch'],
			'lrp_updb_extra_params'     => isset($request['lrp_updb_extra_params']) ? base64_encode(json_encode($request['lrp_updb_extra_params'])) : base64_encode(json_encode(array())),
			'lrp_updb_nonce'            => wp_create_nonce('tpupdatedatabase'),
			'lrp_update_completed'      => 'no',
	 ...
	 * @return bool
	 */
	public function lrp_updated_database_full_trim_originals_140( $language_code, $inferior_limit, $batch_size ){
		if ( ! $this->lrp_query ) {
			$lrp = LRP_Lingua_Press::get_lrp_instance();
			/* @var LRP_Query */
			$this->lrp_query = $lrp->get_component( 'query' );
		...
		$strings = $this->lrp_query->get_rows_from_location( $language_code, $inferior_limit, $batch_size, array( 'id', 'original' ) );
		if ( count( $strings ) == 0 ) {
			return true;
		}
		foreach( $strings as $key => $string ){
			$strings[$key]['original'] = lrp_full_trim( $strings[ $key ]['original'] );
		}

		// overwrite original only
		$this->lrp_query->update_strings( $strings, $language_code, array( 'id', 'original' ) );
		return false;
	}
	 ...
	 * @return bool
	 */
	public function lrp_updated_database_gettext_empty_rows_145( $language_code, $inferior_limit, $batch_size ){
		if ( ! $this->lrp_query ) {
			$lrp = LRP_Lingua_Press::get_lrp_instance();
			/* @var LRP_Query */
			$this->lrp_query = $lrp->get_component( 'query' );
		}
		$rows_affected = $this->lrp_query->delete_empty_gettext_strings( $language_code, $batch_size );
		if ( $rows_affected > 0 ){
			return false;
		}else{
			return true;
     ...
     * @return bool
     */
    public function lrp_updated_database_original_id_insert_166( $language_code, $inferior_limit, $batch_size ){
        if ( ! $this->lrp_query ) {
            $lrp = LRP_Lingua_Press::get_lrp_instance();
            /* @var LRP_Query */
            $this->lrp_query = $lrp->get_component( 'query' );
        ...
        $rows_inserted = $this->lrp_query->original_ids_insert( $language_code, $inferior_limit, $batch_size );
    ...
    public function lrp_updated_database_original_id_cleanup_166( $language_code, $inferior_limit, $batch_size ){
        if ( ! $this->lrp_query ) {
            $lrp = LRP_Lingua_Press::get_lrp_instance();
            /* @var LRP_Query */
            $this->lrp_query = $lrp->get_component( 'query' );
        ...
        $this->lrp_query->original_ids_cleanup();
     ...
     * @return bool
     */
    public function lrp_updated_database_original_id_update_166( $language_code, $inferior_limit, $batch_size ){
        if ( ! $this->lrp_query ) {
            $lrp = LRP_Lingua_Press::get_lrp_instance();
            /* @var LRP_Query */
            $this->lrp_query = $lrp->get_component( 'query' );
        ...
        $rows_updated = $this->lrp_query->original_ids_reindex( $language_code, $inferior_limit, $batch_size );
    ...
    public function lrp_prepare_options_for_database_optimization(){
        if ( !current_user_can( 'manage_options' ) || !isset( $_GET['lrp_rm_nonce'] ) ) {
            return;
        }

        if ( !wp_verify_nonce( sanitize_text_field( $_GET['lrp_rm_nonce'] ), 'tpremoveduplicaterows' ) ){
            exit;
        }

        $redirect = false;

        if(isset( $_GET['lrp_rm_duplicates_gettext'] )){
            update_option('lrp_remove_duplicate_gettext_rows', 'no');
            $redirect = true;
        }

        if(isset( $_GET['lrp_rm_duplicates_dictionary'] )){
            update_option('lrp_remove_duplicate_dictionary_rows', 'no');
            update_option('lrp_remove_duplicate_untranslated_dictionary_rows', 'no');
            $redirect = true;
        }

        if ( isset( $_GET['lrp_rm_duplicates_original_strings'] ) ){
            $this->lrp_remove_duplicate_original_strings();
            $redirect = true;
        }

        if ( isset( $_GET['lrp_rm_cdata_original_and_dictionary'])){
            update_option('lrp_remove_cdata_original_and_dictionary_rows', 'no');
            $redirect = true;
        }

        if ( isset( $_GET['lrp_rm_untranslated_links'] ) ){
            update_option('lrp_remove_untranslated_links_dictionary_rows', 'no');
            $redirect = true;
        }

        if ( isset( $_GET['lrp_replace_original_id_null'] ) ){
            update_option('lrp_replace_original_id_null', 'no');
            $redirect = true;
        }

        if ( $redirect ) {
            $url = add_query_arg( array( 'page' => 'lrp_update_database' ), site_url( 'wp-admin/admin.php' ) );
	...
	/**
	 * Remove duplicate rows from DB for lrp_dictionary tables.
	 ...
	 * http://example.com/wp-admin/admin.php?page=lrp_remove_duplicate_rows
	 */
	public function lrp_remove_duplicate_rows(){
		if ( ! current_user_can( 'manage_options' ) ){
			return;
		}
		// prepare page structure

		require_once LRP_PLUGIN_DIR . 'partials/lrp-remove-duplicate-rows.php';
	}

	public function enqueue_update_script( $hook ) {
		if ( $hook === 'admin_page_lrp_update_database' ) {
			wp_enqueue_script( 'lrp-update-database', LRP_PLUGIN_URL . 'assets/js/lrp-update-database.js', array(
				'jquery',
			), LRP_PLUGIN_VERSION );
		}

		wp_localize_script( 'lrp-update-database', 'lrp_updb_localized ', array(
	...
	private function add_full_text_index_to_tables(){
	    $table_names = $this->lrp_query->get_all_table_names('', array());
        $gettext_table_names = $this->lrp_query->get_all_gettext_table_names();
    ...
    /**
     * Moving some settings from lrp_settings option to lrp_machine_translation_settings
     *
    ...
    private function upgrade_machine_translation_settings(){
        ...
        $machine_translation_settings = get_option( 'lrp_machine_translation_settings', false );

        $default_machine_translation_settings = $lrp_settings->get_default_lrp_machine_translation_settings();
    ...
    private function set_force_slash_at_end_of_links(){
        ...
        if( !empty( $settings['lrp_advanced_settings'] ) && !isset( $settings['lrp_advanced_settings']['force_slash_at_end_of_links'] ) ){
            $advanced_settings = $settings['lrp_advanced_settings'];
            $advanced_settings['force_slash_at_end_of_links'] = 'yes';
            update_option('lrp_advanced_settings', $advanced_settings );
        }
    ...
    public function add_iso_code_to_language_code(){
        ...
        if(isset($settings['lrp_advanced_settings']) && isset($settings['lrp_advanced_settings']['custom_language']) ){
            $advanced_settings = $settings['lrp_advanced_settings'];
    ...
    public function call_create_menu_entries(){
        ...
        $lrp_settings->create_menu_entries( $settings['publish-languages'] );
    }

    public function lrp_remove_duplicate_original_strings(){
        if ( ! $this->lrp_query ) {
            $lrp = LRP_Lingua_Press::get_lrp_instance();
            /* @var LRP_Query */
            $this->lrp_query = $lrp->get_component( 'query' );
        }
        $this->lrp_query->rename_originals_table();
        $this->lrp_query->check_original_table();

        update_option( 'lrp_updated_database_original_id_insert_166', 'no' );
        update_option( 'lrp_updated_database_original_id_cleanup_166', 'no' );
        update_option( 'lrp_updated_database_original_id_update_166', 'no' );

        update_option( 'lrp_regenerate_original_meta_table', 'no' );
        update_option( 'lrp_clean_original_meta_table', 'no' );

    }

    public function lrp_regenerate_original_meta_table($language_code, $inferior_limit, $batch_size ){
        ...
        $this->lrp_query->regenerate_original_meta_table($inferior_limit, $batch_size);

        $last_id = $this->db->get_var("SELECT MAX(meta_id) FROM " .  $this->lrp_query->get_table_name_for_original_meta() );
    ...
    public function lrp_clean_original_meta_table($language_code, $inferior_limit, $batch_size){
        if ( $language_code != $this->settings['default-language']) {
            // perform regeneration of original meta table only once
            return true;
        }
        ...
        $rows_affected = $this->lrp_query->clean_original_meta( $batch_size );
        if ( $rows_affected > 0 ){
            return false;
        }else{
            $old_originals_table = get_option( 'lrp_original_strings_table_for_recovery', '' );
            if ( !empty ( $old_originals_table) && strpos($old_originals_table, 'lrp_original_strings1') !== false ) {
                delete_option('lrp_original_strings_table_for_recovery');
                $this->lrp_query->drop_table( $old_originals_table );
            }
            return true;
        }
    }
     ...
     * @return bool
     */
    public function lrp_updated_database_gettext_original_id_insert( $language_code, $inferior_limit, $batch_size ){
        if ( ! $this->lrp_query ) {
            $lrp = LRP_Lingua_Press::get_lrp_instance();
            /* @var LRP_Query */
            $this->lrp_query = $lrp->get_component( 'query' );
        ...
        $last_id = $this->lrp_query->get_last_id( $this->lrp_query->get_gettext_table_name($language_code) );
     ...
     * @return bool
     */
    public function lrp_updated_database_gettext_original_id_cleanup( $language_code, $inferior_limit, $batch_size ){
        if ( ! $this->lrp_query ) {
            $lrp = LRP_Lingua_Press::get_lrp_instance();
            /* @var LRP_Query */
            $this->lrp_query = $lrp->get_component( 'query' );
     ...
     * @return bool
     */
    public function lrp_updated_database_gettext_original_id_update( $language_code, $inferior_limit, $batch_size ){
        if ( ! $this->lrp_query ) {
            $lrp = LRP_Lingua_Press::get_lrp_instance();
            /* @var LRP_Query */
            $this->lrp_query = $lrp->get_component( 'query' );
    ...
    public function show_notification_about_add_ons_removal(){
        ...
        foreach( $old_addon_list as $addon_slug ) {
            if (is_plugin_active($addon_slug)) {
                ...
                $notification_id = 'lrp_add_ons_removal';

                $url_info = 'https://linguapress.com/docs/installation/upgrade-to-version-2-0-5-or-newer/';
                $url_account = 'https://linguapress.com/account/';
                $message = '<p style="padding-right:30px;">' . sprintf(__( 'All individual LinguaPress add-on plugins <a href="%1$s" target="_blank">have been discontinued</a> and are now included in the premium Personal, Business and Developer versions of LinguaPress. Please log into your <a href="%2$s" target="_blank">account page</a>, download the new premium version and install it. Your individual addons settings will be ported over.' , 'linguapress' ), esc_url($url_info), esc_url($url_account)) . '</p>';
    ...
    * We do this by getting the id from the wp_lrp_gettext_original_strings and updating the wp_lrp_gettext_current_language table with the original ids.
    */

    public function lrp_replace_original_id_null($language_code, $inferior_limit, $batch_size){
        global $wpdb;
        $db       = $wpdb;
        ...
        while ( $last_id > $inferior_limit ) {
            $dictionary = $this->lrp_query->get_all_gettext_strings($language_code, $inferior_limit, $batch_size);
     ...
     */
    public function lrp_get_table_name_for_original_join_in_meta_based_slugs( $meta_type ){
     ...
     */
    public function lrp_get_meta_based_slug_status( $meta_values ) {
     ...
     */
    public function lrp_get_original_slugs_for_meta_based_slugs( $meta_type, $meta_slugs_and_ids, $language_code ) {
            ...
            foreach ( $meta_slugs_and_ids as $meta_values ) {
                ...
                if ( !empty( $meta_values['post_name'] ) ) {
                    ...
                    if ( isset( $meta_values['meta_value'] ) ) {
                        ...
                        $extracted_slugs_array[ $meta_values['post_name'] ]["status"]     = $this->lrp_get_meta_based_slug_status( $meta_values );
            ...
            foreach ( $meta_slugs_and_ids as $meta_values ) {
                ...
                if ( !empty( $meta_values['slug'] ) ) {
                    ...
                    if ( isset( $meta_values['meta_value'] ) ) {
                        ...
                        $extracted_slugs_array[ $meta_values['slug'] ]["status"]     = $this->lrp_get_meta_based_slug_status( $meta_values );
    ...
    public function get_last_id_for_meta_based_slugs( $meta_type, $language_code ){
        ...
        $prepared_query      = $wpdb->prepare( $select_query, '%' . $wpdb->esc_like( 'lrp_automatically_translated_slug_' . $language_code ) . '%', '%' . $wpdb->esc_like( 'lrp_translated_slug_'. $language_code ) . '%' );
     ...
     */
    public function lrp_get_meta_based_slugs_from_db_284( $meta_type, $language_code, $inferior_limit, $batch_size ) {
        global $wpdb;

        $table_name_for_originals_for_the_selected_meta_type = $this->lrp_get_table_name_for_original_join_in_meta_based_slugs( $meta_type );
        $extracted_slugs_array = array();

        if ( $meta_type == 'postmeta' ) {

            $select_query          = "SELECT meta_key, meta_value, lrp_original_name.post_name FROM `" . $wpdb->postmeta . "` as lrp_translation ";
            $select_query          .= "INNER JOIN `" . $wpdb->posts . "` as lrp_original_name ";
            $select_query          .= "ON lrp_translation.post_id = lrp_original_name.ID ";
            $select_query          .= "WHERE ( meta_key LIKE %s OR meta_key LIKE %s ) LIMIT %d OFFSET %d";
            $prepared_query        = $wpdb->prepare( $select_query, '%' . $wpdb->esc_like( 'lrp_automatically_translated_slug_' . $language_code ) . '%', '%' . $wpdb->esc_like( 'lrp_translated_slug_' . $language_code ) . '%', $batch_size, $inferior_limit );
            $meta_slugs_and_ids    = $wpdb->get_results( $prepared_query, 'ARRAY_A' );

            $extracted_slugs_array = $this->lrp_get_original_slugs_for_meta_based_slugs( 'postmeta', $meta_slugs_and_ids, $language_code );

        }elseif ( $meta_type == 'termmeta' ) {

            $select_query          = "SELECT meta_key, meta_value, lrp_original_name.slug FROM `" . $wpdb->termmeta . "` as lrp_translation ";
            $select_query          .= "INNER JOIN `" . $wpdb->terms . "` as lrp_original_name ";
            $select_query          .= "ON lrp_translation.term_id = lrp_original_name.term_id ";
            $select_query          .= "WHERE ( meta_key LIKE %s OR meta_key LIKE %s ) LIMIT %d OFFSET %d";
            $prepared_query        = $wpdb->prepare( $select_query, '%' . $wpdb->esc_like( 'lrp_automatically_translated_slug_' . $language_code ) . '%', '%' . $wpdb->esc_like( 'lrp_translated_slug_' . $language_code ) . '%', $batch_size, $inferior_limit );
            $meta_slugs_and_ids    = $wpdb->get_results( $prepared_query, 'ARRAY_A' );

            $extracted_slugs_array = $this->lrp_get_original_slugs_for_meta_based_slugs( 'termmeta', $meta_slugs_and_ids, $language_code );
     ...
     *
     * LOGUSTIC: - the oprion based slugs are stored in the wp_options table under the option name of * lrp_taxonomy_slug_translation
     ...
     */
    public function lrp_get_option_based_slugs_from_db_284( $option_name ) {
        ...
        foreach ( $data as $key => $values_array ) {
            ...
            if ( $values_array['type'] == 'post-type-base-slug' || $values_array['type'] == 'post-type-base' ) {
                ...
                if ( apply_filters( 'lrp_migrate_post_type_or_tax_original_only_if_active', true, $extracted_slugs_array[ $key ]["original"], $values_array['type'] ) && (
     ...
     *
     */
    public function lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_post_type_and_tax_284() {
        if ( class_exists( 'LRP_Slug_Query' ) ) {
            ...
            $extracted_slugs_array                         = array();
            $extracted_slugs_array['taxonomy_slugs']       = $this->lrp_get_option_based_slugs_from_db_284( 'lrp_taxonomy_slug_translation' );
            $extracted_slugs_array['post_type_base_slugs'] = $this->lrp_get_option_based_slugs_from_db_284( 'lrp_post_type_base_slug_translation' );
            ...
            update_option( 'lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_post_type_and_tax_284', 'seopack_inactive' );
            return true;
        }
    }
     ...
     * Using batches
     */
    public function lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_post_meta_284( $language_code, $inferior_limit, $batch_size ) {
        if ( class_exists( 'LRP_Slug_Query' ) ) {
            ...
            $extracted_slugs_array_post = $this->lrp_get_meta_based_slugs_from_db_284( 'postmeta', $language_code, $inferior_limit, $batch_size );
            ...
            update_option( 'lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_post_meta_284', 'seopack_inactive' );
            return true;
        }
    }
     ...
     * Using batches
     */
    public function lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_term_meta_284( $language_code, $inferior_limit, $batch_size ) {
        ...
        if ( class_exists( 'LRP_Slug_Query' ) ) {
            ...
            $extracted_slugs_array_term = $this->lrp_get_meta_based_slugs_from_db_284( 'termmeta', $language_code, $inferior_limit, $batch_size );
            ...
            update_option( 'lrp_migrate_old_slug_to_new_parent_and_translate_slug_table_term_meta_284', 'seopack_inactive' );
            return true;
        }
    }
    ...
    public function migrate_machine_translation_counter() {
        $mt_settings_option = get_option( 'lrp_machine_translation_settings', null );
        if ( $mt_settings_option && isset( $mt_settings_option['machine_translation_counter'] ) ) {
            update_option( 'lrp_machine_translation_counter', $mt_settings_option['machine_translation_counter'] );
        }
    }
    ...
    public function is_seo_pack_minimum_version_met(){
        if ( !defined('LRP_IN_SP_PLUGIN_VERSION' ) ) return true;

        return version_compare( LRP_IN_SP_PLUGIN_VERSION, self::MINIMUM_SP_VERSION, '>=' );
    }
    ...
    public function is_pro_minimum_version_met(){
        ...
        if ( defined( 'LRP_IN_EL_PLUGIN_URL' ) && file_exists( LRP_IN_EL_PLUGIN_DIR . 'assets/js/lrp-back-end-script-pro.js' )){
            return true;
        }

        //In case extra-languages addon is not active we check with the paths hardcoded
        $pro_version_map = [
            'LinguaPress - Personal'  => 'linguapress-personal',
            'LinguaPress - Business'  => 'linguapress-business',
    ...
    public function show_admin_notice_minimum_pro_version_required(){
        //show this only on our linguapress admin pages
        if( isset( $_GET['page'] ) && ( sanitize_text_field( $_GET['page'] ) === 'lingua-press' || strpos( sanitize_text_field( $_GET['page'] ), 'lrp_' ) !== false ) ){
    ...
    public function set_publish_languages_from_translation_languages() {
        $extra_languages_is_active = class_exists( 'LRP_IN_Extra_Languages' );
        $lrp_settings              = get_option( 'lrp_settings' );
        if ( !$extra_languages_is_active &&
            count( $lrp_settings['translation-languages'] ) <= 2 &&
            ( empty( array_diff( $lrp_settings['translation-languages'], $lrp_settings['publish-languages'] ) ) ||
            empty( array_diff( $lrp_settings['publish-languages'], $lrp_settings['translation-languages'] ) ) )
        ) {
            $lrp_settings['publish-languages'] = $lrp_settings['translation-languages'];
            update_option( 'lrp_settings', $lrp_settings );
        }
    }
