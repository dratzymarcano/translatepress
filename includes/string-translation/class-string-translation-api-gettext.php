...
class LRP_String_Translation_API_Gettext {
	...
	public function get_strings(){
		...
		$originals_results = $this->helper->get_originals_results(
			$this->type,
			$lrp_query->get_table_name_for_gettext_original_strings(),
			$lrp_query->get_table_name_for_gettext_original_meta(),
        ...
        if ( !empty( $query_args['s'] ) )
            set_transient( 'lrp_gettext_search', $query_args['s'], 10 );
	...
	public function get_missing_gettext_strings() {
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			...
			if ( isset( $_POST['action'] ) && $_POST['action'] === $action && isset( $_POST['original_ids'] ) && isset( $_POST['lrp_ajax_language'] ) ) {
				...
				$lrp_ajax_language = sanitize_text_field( $_POST['lrp_ajax_language'] );
				...
				if ( in_array( $lrp_ajax_language, $settings['translation-languages'] ) ) {
					$language = $lrp_ajax_language;
				} else {
					wp_die();
				}

				$dictionary      = $lrp_query->get_gettext_string_rows_by_original_id( $original_ids, $language );
	...
	public function get_strings_by_original_ids(){
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			...
			$action = 'lrp_string_translation_get_strings_by_original_ids_gettext';
			if ( isset( $_POST['action'] ) && $_POST['action'] === $action && isset( $_POST['original_ids'] ) ) {
				...
				foreach ( $settings['translation-languages'] as $language ) {
					$dictionaries[ $language ] = $lrp_query->get_gettext_string_rows_by_original_id( $original_ids, $language );
				}
				...
				$translation_manager = $lrp->get_component('translation_manager');
				...
				$dictionary_by_original = lrp_sort_dictionary_by_original( $dictionaries, 'gettext', $localized_text['gettextstrings'], $post_language );

                $search_query = get_transient('lrp_gettext_search' );
				...
				echo lrp_safe_json_encode( array('dictionary' => $dictionary_by_original ) ); //phpcs:ignore
    ...
    public function delete_strings() {
        ...
        $regular_delete = new LRP_Gettext_Delete();
