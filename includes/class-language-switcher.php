...
class LRP_Language_Switcher{
    ...
    /**
     * LRP_Language_Switcher constructor.
     *
     * @param array $settings           Settings option.
     * @param $lrp LRP_Lingua_Press  Trp object
     */
    public function __construct( $settings, $lrp ){
        $this->settings = $settings;
        $this->lrp = $lrp;
        $this->url_converter = $this->lrp->get_component( 'url_converter' );
        $language = $this->get_current_language($lrp);
        global $LRP_LANGUAGE;
     ...
     *
	 * @param $lrp LRP_Lingua_Press  LRP singleton object
	 *
	 * @return string       Language code
	 */
	private function get_current_language( $lrp ){
		$language_from_url = $this->url_converter->get_lang_from_url_string();

		$needed_language = $this->determine_needed_language( $language_from_url, $lrp );

		$allow_redirect = apply_filters( 'lrp_allow_language_redirect', true, $needed_language, $this->url_converter->cur_page_url() );
		if ( $allow_redirect ) {
			if ( ( $language_from_url == null && isset( $this->settings['add-subdirectory-to-default-language'] ) && $this->settings['add-subdirectory-to-default-language'] == 'yes' ) ||
                ...
                global $LRP_NEEDED_LANGUAGE;
                $LRP_NEEDED_LANGUAGE = $needed_language;
	 ...
	 * @param LRP_Lingua_Press $lrp       LRP singleton object
	 *
	 * @return string Language code
	 */
	public function determine_needed_language( $lang_from_url, $lrp ){
		...
		return apply_filters( 'lrp_needed_language', $needed_language, $lang_from_url, $this->settings, $lrp );
    }

	/**
	 * Redirects to language stored in global $LRP_NEEDED_LANGUAGE
	 */
	public function redirect_to_correct_language(){
        ...
        $link_to_redirect = sanitize_url(apply_filters( 'lrp_link_to_redirect_to', $this->url_converter->get_url_for_language( $LRP_NEEDED_LANGUAGE, null, '' ), $LRP_NEEDED_LANGUAGE ));

        if( isset( $this->settings['add-subdirectory-to-default-language'] ) && $this->settings['add-subdirectory-to-default-language'] === 'yes' && isset( $this->settings['default-language'] ) && $this->settings['default-language'] === $LRP_NEEDED_LANGUAGE ) {
            $status = apply_filters( 'lrp_redirect_status', 301, 'redirect_to_add_subdirectory_to_default_language' );
            wp_redirect( $link_to_redirect, $status );
        }else {
            $status = apply_filters( 'lrp_redirect_status', 302, 'redirect_to_a_different_language_according_to_url_slug' );
            wp_redirect( $link_to_redirect, $status );
        }
	...
	public function language_switcher( $atts ){
		...
		$current_language = apply_filters('lrp_ls_shortcode_current_language', $current_language, $published_languages, $LRP_LANGUAGE, $this->settings);
		$other_languages = apply_filters('lrp_ls_shortcode_other_languages', $other_languages, $published_languages, $LRP_LANGUAGE, $this->settings);

		if( ! $this->lrp_settings_object ) {
			$lrp = LRP_Lingua_Press::get_lrp_instance();
			$this->lrp_settings_object = $lrp->get_component( 'settings' );
		}
		$ls_options = $this->lrp_settings_object->get_language_switcher_options();
		...
		require LRP_PLUGIN_DIR . 'partials/language-switcher-shortcode.php';
    ...
    public function enqueue_language_switcher_scripts( ) {
        ...
        if ( apply_filters( 'lrp_enqueue_style_language_switcher_css', true ) ) {

            if ( isset( $this->settings['lrp-ls-floater'] ) && $this->settings['lrp-ls-floater'] == 'yes' ) {
                $floater_path = apply_filters( 'lrp_old_css_styling_for_floater_ls', false ) ? LRP_PLUGIN_URL . 'assets/css/lrp-floater-language-switcher-old.css' : LRP_PLUGIN_URL . 'assets/css/lrp-floater-language-switcher.css';
                wp_enqueue_style('lrp-floater-language-switcher-style', $floater_path, array(), LRP_PLUGIN_VERSION);
            }

            $shortcode_path = apply_filters( 'lrp_old_css_styling_for_shortcode_ls', false ) ? LRP_PLUGIN_URL . 'assets/css/lrp-language-switcher-old.css' : LRP_PLUGIN_URL . 'assets/css/lrp-language-switcher.css';
            wp_enqueue_style( 'lrp-language-switcher-style', $shortcode_path, array(), LRP_PLUGIN_VERSION );
        }
    }
    ...
    public function add_floater_language_switcher() {
        ...
        if( $this->settings['lrp-ls-floater'] != 'yes' ) {
            return;
        }
        ...
        $floater_class = 'lrp-floater-ls-names';
        $floater_flags_class = '';
        ...
        if( $floater_settings['short_names'] ) {
            $floater_class  = 'lrp-floater-ls-codes';
        }

        if( $floater_settings['flags'] && ! $floater_settings['full_names'] && ! $floater_settings['short_names'] ) {
            $floater_class  = 'lrp-floater-ls-flags';
        }

        if( $floater_settings['flags'] && ( $floater_settings['full_names'] || $floater_settings['short_names'] ) ) {
            $floater_flags_class = 'lrp-with-flags';
        }

        if( $this->settings['floater-position'] ) {
            $floater_class .= ' lrp-' . esc_attr($this->settings['floater-position']);
        }

        if( $this->settings['floater-color'] ) {
		        $floater_class .= ' lrp-color-' . esc_attr($this->settings['floater-color']);
	      } else {
	          $floater_class .= ' lrp-color-dark'; // default color. Good for backwards compatibility as well.
        }

        if( $this->settings['lrp-ls-show-poweredby'] == 'yes' ) {
            $floater_class .= ' lrp-poweredby';
        }
        ...
        $current_language = apply_filters('lrp_ls_floating_current_language', $current_language, $published_languages, $LRP_LANGUAGE, $this->settings);
        $other_languages = apply_filters('lrp_ls_floating_other_languages', $other_languages, $published_languages, $LRP_LANGUAGE, $this->settings);
        ...
        ?>
        <div id="lrp-floater-ls" onclick="" data-no-translation class="lrp-language-switcher-container <?php echo esc_attr( $floater_class ); ?>" <?php echo ( isset( $_GET['lrp-edit-translation'] ) && $_GET['lrp-edit-translation'] == 'preview' ) ? 'data-lrp-unpreviewable="lrp-unpreviewable"' : '' ?>>
            <div id="lrp-floater-ls-current-language" class="<?php echo esc_attr( $floater_flags_class ); ?>">

                <a href="#" class="lrp-floater-ls-disabled-language lrp-ls-disabled-language" onclick="event.preventDefault()">
            ...
            <div id="lrp-floater-ls-language-list" class="<?php echo esc_attr( $floater_flags_class );?>" <?php echo ( isset( $_GET['lrp-edit-translation'] ) && $_GET['lrp-edit-translation'] == 'preview' ) ? 'data-lrp-unpreviewable="lrp-unpreviewable"' : ''?>>

                <?php
                if( $this->settings['lrp-ls-show-poweredby'] == 'yes' ){
	                $powered_by = '<div id="lrp-floater-poweredby">Powered by <a href="https://linguapress.com/?utm_source=language_switcher&utm_medium=clientsite&utm_campaign=TPLS" rel="nofollow" target="_blank" title="WordPress Translation Plugin">LinguaPress</a></div>';
                } else {
	                $powered_by = '';
                }

                if ( apply_filters('lrp_ls_floater_show_disabled_language', true, $current_language, $this->settings ) ) {
                    $disabled_language = '<a href="#" class="lrp-floater-ls-disabled-language lrp-ls-disabled-language" onclick="event.preventDefault()">';
                ...
                if ( !empty( $this->settings['floater-position'] ) && strpos( $this->settings['floater-position'], 'top' ) !== false  ){
	                  ...
	                  echo '<div class="lrp-language-wrap lrp-language-wrap-top">';
                ...
                if ( $floater_position == 'bottom' ){
                    echo '<div class="lrp-language-wrap lrp-language-wrap-bottom">';
	              }
                ...
                foreach( $other_languages as $code => $name ) {
                    ...
                    <a href="<?php echo esc_url( $this->url_converter->get_url_for_language($code, false) ); ?>"
                        <?php echo ( isset( $_GET['lrp-edit-translation'] ) && $_GET['lrp-edit-translation'] == 'preview' ) ? 'data-lrp-unpreviewable="lrp-unpreviewable"' : '' ?> title="<?php echo esc_attr( $name ); ?>">
          						  <?php
    ...
    <?php
        ...
        echo apply_filters( 'lrp_floating_ls_html', $floating_ls_html ); // phpcs:ignore
    }
    ...
    public function add_flag( $language_code, $language_name, $location = NULL ) {
        ...
        $flags_path = apply_filters( 'lrp_flags_path', $flags_path, $language_code );

        // File name for specific flag
        $flag_file_name = $language_code .'.png';
        $flag_file_name = apply_filters( 'lrp_flag_file_name', $flag_file_name, $language_code );

        // HTML code to display flag image
        $flag_html = '<img class="lrp-flag-image" src="'. esc_url( $flags_path . $flag_file_name ) .'" width="18" height="12" alt="' . esc_attr( $language_code ) . '" title="' . esc_attr( $language_name ) . '">';
    ...
    public function ls_menu_permalinks( $items, $menu, $args ){
        ...
        $published_languages = $this->lrp_languages->get_language_names( $lrp_settings->get_settings()['publish-languages'] );
        ...
        foreach ( $items as $key => $item ){
            if ( $item->object == 'language_switcher' ){
                ...
                if ( !isset( $language_labels[ $language_code ] ) ) {
                    ...
                    $language_names = $this->lrp_languages->get_language_names( array( $language_code ) );
                ...
                if ($menu_settings["no_html"] ){
	                ...
	                $items[$key]->classes[] = 'lrp-language-switcher-container';
	                ...
	                if ( $menu_settings['short_names'] ) {
		                $items[$key]->title .= '<span class="lrp-ls-language-name">' . strtoupper( $this->url_converter->get_url_slug( $language_code, false ) ) . '</span>';
	                }
	                if ( $menu_settings['full_names'] ) {
		                $items[$key]->title .= '<span class="lrp-ls-language-name">' . wp_kses_post( $language_name ) . '</span>';
	                }
	                $items[$key]->title .= '</span>';

                }

                $items[$key]->title = apply_filters( 'lrp_menu_language_switcher', $items[$key]->title, $language_name, $language_code, $menu_settings );
            }
        }
