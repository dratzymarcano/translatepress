...
add_image_size( 'lrp-custom-language-flag', 18, 12 );

// Register country flag size for use in Add Media modal
add_filter( 'image_size_names_choose', 'lrp_add_flag_sizes' );
function lrp_add_flag_sizes( $sizes ) {
	return array_merge( $sizes, array(
		'lrp-custom-language-flag' => __( 'Custom Language Flag', 'linguapress' )
	) );
}

add_filter( 'lrp_wp_languages', 'lrpc_add_custom_language', 10, 2 );
function lrpc_add_custom_language( $languages ) {
	...
	if ( isset( $option['custom_language'] ) ) {
		...
		foreach ( $option['custom_language']['cuslangname'] as $key => $value ) {
			...
			if ( isset( $option["cuslangisrtl"] ) && $option["cuslangisrtl"] === 'yes' && $LRP_LANGUAGE === $custom_language_iso ) {
...
add_filter('gettext_with_context', 'lrpc_language_rtl', 10, 4);
function lrpc_language_rtl($translated, $text, $context, $domain){
	$option = get_option( 'lrp_advanced_settings', true );
	global $LRP_LANGUAGE;
	...
	if ( isset( $option['custom_language'] ) ) {
		foreach ( $option['custom_language']['cuslangname'] as $key => $value ) {
			...
			if($text == 'ltr' && $context == "text direction" && isset($option["custom_language"]["cuslangisrtl"][$key]) && $option["custom_language"]["cuslangisrtl"][$key] === 'yes' && $LRP_LANGUAGE === $custom_language_code){
				$translated = 'rtl';
			}
		}
	}
...
add_filter( 'lrp_flags_path', 'lrpc_flags_path_custom', 10, 2 );
/**
 ...
 * The image is returned resized to the custom size dictated bu lrp-custom-language-flag
 *
 */
function lrpc_flags_path_custom( $original_flags_path,  $language_code ) {
	...
	if ( isset( $option['custom_language'] ) ) {
		foreach ( $option['custom_language']['cuslangname'] as $key => $value ) {
			if ($language_code === $option["custom_language"]["cuslangcode"][$key] && !empty($option["custom_language"]["cuslangflag"][$key]) ) {
				$attachment_array = wp_get_attachment_image_src(attachment_url_to_postid($option["custom_language"]["cuslangflag"][ $key ]), 'lrp-custom-language-flag');
...
add_filter( 'lrp_flag_file_name', 'lrpc_flag_name_custom', 10, 2 );
/**
 * @param $original_flags_path
 * @param $language_code
 *
 ...
 */
function lrpc_flag_name_custom ( $original_flags_path,  $language_code ){
...
add_filter('lrp_saving_advanced_settings_is_successful', 'lrp_add_messages_custom_language_codes', 10, 3);
...
function lrp_verify_custom_language_codes($is_correct_code, $settings){
    ...
    if(isset($settings['custom_language']['cuslangcode'])) {
        foreach ($settings['custom_language']['cuslangcode'] as $key => $item) {
            if (!empty($settings['custom_language']['cuslangcode'][$key])) {
                if (!lrp_is_valid_language_code($item)) {
                    $is_correct_code = false;

                    return array(
                        'message' => esc_html__('The Language code of the added custom language is invalid.','linguapress'),
                ...
                $is_correct_code = false;
                return array(
                    'message'      => esc_html__('The Language code of the added custom language cannot be empty.', 'linguapress'),
                    'correct_code' => $is_correct_code
                );
            }
        }
        ...
        foreach ($settings['custom_language']['cuslangiso'] as $key => $item) {
            if(!empty($settings['custom_language']['cuslangiso'][$key])){
                if (!lrp_is_valid_language_code($item)) {
                    ...
                    return array(
                        'message' => esc_html__('The Automatic Translation Code of the added custom language is invalid.', 'linguapress'),
                        'correct_code' => $is_correct_code
                    );
                }
            }
...
function lrp_add_messages_custom_language_codes($correct_code, $settings, $submitted_settings){

    $correct_code_custom_language = lrp_verify_custom_language_codes(true, $settings);

    if($correct_code_custom_language['correct_code'] === false){
        /* phpcs:ignore */
        add_settings_error( 'lrp_advanced_settings', 'settings_error', esc_html($correct_code_custom_language['message']), 'error' );
        $correct_code = false;
...
add_filter('lrp_extra_sanitize_advanced_settings', 'lrp_save_settings_language', 10, 3);
...
function lrp_save_settings_language($settings, $submitted_settings, $prev_settings){

    $correct_custom_languagea_code = lrp_verify_custom_language_codes(true, $settings);
