    ...
    function add_admin_notice() {
        ...
        if ( current_user_can( 'manage_options' ) ){
            ...
            if ( ! get_user_meta($user_id, $this->notificationId.'_dismiss_notification' ) || $this->force_show  ) {//ignore the dismissal if we have force_show
                ...
                echo wp_kses( apply_filters($this->notificationId.'_notification_message','<div class="'. $this->notificationClass .'" style="position:relative;'  . ((strpos($this->notificationClass, 'lrp-narrow')!==false ) ? 'max-width: 825px;' : '') . '" >'.$this->notificationMessage.'</div>', $this->notificationMessage), [ 'div' => [ 'class' => [],'style' => [] ], 'p' => ['style' => [], 'class' => []], 'a' => ['href' => [], 'type'=> [], 'class'=> [], 'style'=>[], 'title'=>[],'target'=>[]], 'span' => ['class'=> []], 'strong' => [], 'img' => [ 'src' => [], 'style' => [] ] ] );
...
Class LRP_Plugin_Notifications {

    public $notifications = array();
    private static $_instance = null;
    private $prefix = 'lrp';
    private $menu_slug = 'options-general.php';
    public $pluginPages = array( 'lingua-press', 'lrp_addons_page', 'lrp_license_key', 'lrp_advanced_page', 'lrp_machine_translation', 'lrp_test_machine_api' );
                            ...
                            foreach ($callbacks_level as $key => $callback) {
                                if( is_array( $callback['function'] ) ){
                                    if( is_object($callback['function'][0])) {//object here
                                        if (strpos(get_class($callback['function'][0]), 'PMS_') !== 0 && strpos(get_class($callback['function'][0]), 'WPPB_') !== 0 && strpos(get_class($callback['function'][0]), 'LRP_') !== 0 && strpos(get_class($callback['function'][0]), 'WCK_') !== 0) {
                                    ...
                                    if (strpos($callback['function'], 'pms_') !== 0 && strpos($callback['function'], 'wppb_') !== 0 && strpos($callback['function'], 'lrp_') !== 0 && strpos($callback['function'], 'wck_') !== 0) {
                                        unset($wp_filter['admin_notices']->callbacks[$priority][$key]);//unset everything that doesn't come from our plugins
                                    }
                                }
                            }
    ...
    public static function get_instance() {
        if( is_null( self::$_instance ) )
            self::$_instance = new LRP_Plugin_Notifications();
    ...
    public function add_notification( $notification_id = '', $notification_message = '', $notification_class = 'update-nag', $count_in_menu = true, $count_in_submenu = array(), $show_in_all_backend = false, $non_dismissable = false ) {
        ...
        if( $this->is_plugin_page() || $show_in_all_backend ) {
            new LRP_Add_General_Notices( $notification_id, $notification_message, $notification_class, '', '', $force_show );
        }
    ...
    public function dismiss_notification( $notification_id = '' ) {
        ...
        do_action('lrp_dismiss_notification', $notification_id, $current_user);
    }
...
class LRP_Trigger_Plugin_Notifications{
    ...
    function add_plugin_notifications() {
        ...
        $tp_product_name = reset($lrp->tp_product_name);
        $free_version = $tp_product_name == 'LinguaPress';

        if ( empty($license_details) && !$is_demosite && !$free_version ){
            /* this must be unique */
            $notification_id = 'lrp_invalid_license';
            $message = '<p style="padding-right:30px;">';
            $message .= sprintf( __('Your <strong>LinguaPress</strong> license key is invalid or missing. <br/>Please %1$sregister your copy%2$s to receive access to automatic updates and support. Need a license key? %3$sPurchase one now%4$s' , 'linguapress' ), "<a href='". admin_url('/admin.php?page=lrp_license_key') ."'>", "</a>", "<a href='https://linguapress.com/pricing/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-SN-Purchase' target='_blank' class='button-primary'>", "</a>" );
            if ( !$notifications->is_plugin_page() ) {
                //make sure to use the lrp_dismiss_admin_notification arg
                $message .= '<a style="text-decoration: none;z-index:100;" href="' . add_query_arg( array( 'lrp_dismiss_admin_notification' => $notification_id ) ) . '" type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__( 'Dismiss this notice.', 'linguapress' ) . '</span></a>';
                $force_show = false;
            } else {
                $force_show = true; //ignore dismissal on own plugin pages
            }
            $message .= '</p>';
            $notifications->add_notification( $notification_id, $message, 'lrp-notice notice error', true, array('lingua-press'), true, $force_show );
        }
        ...
        if( !empty($license_details) && !$is_demosite && !$free_version){
            ...
            if( !empty( $license_details['invalid'] ) ){
                ...
                elseif( $license_detail->error == 'site_inactive' )
                    $message .= __( 'Your license is disabled for this URL. Re-enable it from <a target="_blank" href="https://linguapress.com/account/">https://linguapress.com/account</a> -> Manage Sites.', 'linguapress' );
                elseif( $license_detail->error == 'no_activations_left' )
                    $message .= sprintf( __('You have reached the activation limit for your <strong>%1$s</strong> license. <br/>Manage your active sites from %2$s your account %3$s.' , 'linguapress' ), $tp_product_name, "<a href='https://linguapress.com/account/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-License-Check' target='_blank' >", "</a>" );
                elseif( $license_detail->error == 'item_name_mismatch' )
                    $message .= sprintf( __('License key mismatch. The license you entered doesn’t match the <strong>%1$s</strong> version you have installed. <br/>Please check that you’ve installed the correct version for your license from your %2$sLinguaPress account%3$s.' , 'linguapress' ), $tp_product_name, "<a href='https://linguapress.com/account/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-License-Check' target='_blank' >", "</a>" );
                elseif( $license_detail->error == 'expired' )
                    $message .= sprintf( __('Your <strong>LinguaPress</strong> license has expired. <br/>Please %1$sRenew Your Licence%2$s to continue receiving access to product downloads, automatic updates and support. %3$sRenew now %4$s' , 'linguapress' ), "<a href='https://www.linguapress.com/account/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-Renewal' target='_blank'>", "</a>", "<a href='https://www.linguapress.com/account/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-Renewal' target='_blank' class='button-primary'>", "</a>" );
                else {
                    $license_error = __("Error: ", "linguapress");
                    if (!empty($license_detail->error)){
                        $license_error .= $license_detail->error;
                    }
                    $message .= __('Something went wrong, please try again.', 'linguapress') . $license_error ;
                }
                if ( !$notifications->is_plugin_page() ) {
                    //make sure to use the lrp_dismiss_admin_notification arg
                    $message .= '<a style="text-decoration: none;z-index:100;" href="' . add_query_arg( array( 'lrp_dismiss_admin_notification' => $notification_id ) ) . '" type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__( 'Dismiss this notice.', 'linguapress' ) . '</span></a>';
                ...
                $message .= '</p>';
                if (!isset($_GET['lrp_sl_activation'])) {
                    $notifications->add_notification($notification_id, $message, 'lrp-notice notice error', true, array('lingua-press'), true, $force_show);
                }
                ...
                if( isset( $license_detail->auto_billing ) && !$license_detail->auto_billing ) {//auto_billing was added by us in a filter on linguapress.com
                    if ( ( strtotime($license_detail->expires ) - time() ) / (60 * 60 * 24) < 30 ) {

                        /* this must be unique */
                        $notification_id = 'lrp_will_expire_license';
                        $message = '<p style="padding-right:30px;">' . sprintf( __( 'Your <strong>LinguaPress</strong> license will expire on %1$s. Please %2$sRenew Your Licence%3$s to continue receiving access to product downloads, automatic updates and support.', 'linguapress'), date_i18n( get_option( 'date_format' ), strtotime( $license_detail->expires, current_time( 'timestamp' ) ) ), '<a href="https://linguapress.com/account/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-Renewal" target="_blank">', '</a>'). '</p>';
                        ...
                        if (!isset($_GET['lrp_sl_activation'])) {
                            $notifications->add_notification($notification_id, $message, 'lrp-notice notice notice-info is-dismissible', true, array('lingua-press'), $force_show);
                        }
                    }
                }
            }
        ...
        if( !empty($license_details) && !$is_demosite && $free_version){
            if( !empty( $license_details['invalid'] ) ){
                ...
                if(
                    ...
                    $message .= sprintf( __('You do not have a valid license for <strong>LinguaPress</strong>. %1$sGet one for free%2$s to get access to LinguaPress AI.' , 'linguapress' ), "<a href='https://linguapress.com/ai-free/?utm_source=wpbackend&utm_medium=clientsite&utm_content=license-page&utm_campaign=tpaifree' target='_blank'>", "</a>" );
                elseif( $license_detail->error == 'site_inactive' )
                    $message .= __( 'Your license is disabled for this URL. Re-enable it from <a target="_blank" href="https://linguapress.com/account/">https://linguapress.com/account</a> -> Manage Sites.', 'linguapress' );
                elseif( $license_detail->error == 'no_activations_left' )
                    $message .= sprintf( __('You have reached the activation limit for your <strong>%1$s</strong> license. <br/>Manage your active sites from %2$s your account %3$s.' , 'linguapress' ), $tp_product_name, "<a href='https://linguapress.com/account/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-License-Check' target='_blank' >", "</a>" );
                ...
                elseif( $license_detail->error == 'website_already_on_free_license' )
                    $message .= sprintf( __('This website is already activated under a free license. Each website can only use one free license. Please upgrade to a premium plan for more LinguaPress AI words from %1$s your account %2$s.' , 'linguapress' ),  "<a href='https://linguapress.com/account/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-License-Check' target='_blank' class='button-primary' >", "</a>" );
                elseif( $license_detail->error == 'expired' )
                    $message .= sprintf( __('Your <strong>LinguaPress</strong> license has expired. <br/>Please %1$sRenew Your Licence%2$s to continue receiving access to product downloads, automatic updates and support. %3$sRenew now %4$s' , 'linguapress' ), "<a href='https://www.linguapress.com/account/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-Renewal' target='_blank'>", "</a>", "<a href='https://www.linguapress.com/account/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=TP-Renewal' target='_blank' class='button-primary'>", "</a>" );
                else {
                    $license_error = __(" Error: ", "linguapress");
        ...
        /* this must be unique */
//	    $notification_id = 'lrp_new_feature_image_translation';
//
//	    $message = '<p style="padding-right:30px;">' . __('NEW: Display different images based on language. Find out <a href="https://linguapress.com/docs/image-translation/" >how to translate images, sliders and more</a> from the LinguaPress editor.' , 'linguapress' ) . '</p>';
//	    //make sure to use the lrp_dismiss_admin_notification arg
//	    $message .= '<a href="' . add_query_arg(array('lrp_dismiss_admin_notification' => $notification_id)) . '" type="button" class="notice-dismiss"><span class="screen-reader-text">' . __('Dismiss this notice.', 'linguapress') . '</span></a>';
//
//	    $notifications->add_notification($notification_id, $message, 'lrp-notice lrp-narrow notice notice-info', true, array('lingua-press'));


	    /* String translation */
	    // $notification_id = 'lrp_new_feature_string_translation';

	    // $message = '<p style="padding-right:30px;">' . __('NEW: Translate Emails and other plugin texts using String Translation. Find out <a href="https://linguapress.com/docs/translation-editor/string-translation/?utm_source=wpbackend&utm_medium=clientsite&utm_content=tpsettings&utm_campaign=LRP" >how to search for a specific text to translate</a>.' , 'linguapress' ) . '</p>';
	    // //make sure to use the lrp_dismiss_admin_notification arg
	    // $message .= '<a href="' . add_query_arg(array('lrp_dismiss_admin_notification' => $notification_id)) . '" type="button" class="notice-dismiss"><span class="screen-reader-text">' . __('Dismiss this notice.', 'linguapress') . '</span></a>';
        ...
        if ( ! $this->settings_obj )
            $this->settings_obj = $lrp->get_component( 'settings' );

        if ( ! $this->machine_translator_logger )
            $this->machine_translator_logger = $lrp->get_component( 'machine_translator_logger' );

        if( 'yes' === $this->settings['lrp_machine_translation_settings']['machine-translation'] && $this->machine_translator_logger->quota_exceeded() ) {
            /* this must be unique */
            $notification_id = 'lrp_machine_translation_quota_exceeded_'. date('Ymd');

            $message = '';
            $message .= '<p style="margin-top: 16px;padding-right:30px;">';
                $message .= sprintf( __( 'The daily quota for machine translation characters exceeded. Please check the <strong>LinguaPress -> <a href="%s">Automatic Translation</a></strong> page for more information.', 'linguapress' ), admin_url( 'admin.php?page=lrp_machine_translation' ) );
            $message .= '</p>';
            //make sure to use the lrp_dismiss_admin_notification arg
            $message .= '<a href="' . add_query_arg(array('lrp_dismiss_admin_notification' => $notification_id)) . '" type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__( 'Dismiss this notice.', 'linguapress' ) . '</span></a>';
        ...
        if ($machine_translator != null ) {
            if ( apply_filters( 'lrp_mt_available_supported_languages_show_notice', true, $this->settings['translation-languages'], $this->settings ) &&
                'yes' === $this->settings['lrp_machine_translation_settings']['machine-translation'] &&
                !$machine_translator->check_languages_availability( $this->settings['translation-languages'] )
                && $correct_key
            ) {
                /* this must be unique */
                $notification_id = 'lrp_mt_unsupported_languages';

                $message = '<p style="margin-top: 16px;padding-right:30px;">';
                $message .= sprintf( __( 'One or more languages are unsupported by the automatic translation provider. Please check the <strong>LinguaPress -> <a href="%s">Automatic Translation</a></strong> page for more information.', 'linguapress' ), admin_url( 'admin.php?page=lrp_machine_translation#lrp_unsupported_languages' ) );
                $message .= '</p>';
                //make sure to use the lrp_dismiss_admin_notification arg
                $message .= '<a href="' . add_query_arg( array( 'lrp_dismiss_admin_notification' => $notification_id ) ) . '" type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__( 'Dismiss this notice.', 'linguapress' ) . '</span></a>';
        ...
        if( lrp_bf_show_promotion() ){
            ...
            if( $notifications->is_plugin_page() ){
                ...
                $notification_id = 'lrp_bf_2025';

                $message = '<img style="max-width: 60px;" src="' . LRP_PLUGIN_URL . 'assets/images/tp-logo.png" />';
                ...
                if ( !$free_version && $license_status == 'expired' ){
                    ...
                    $message .= '<p style="font-size: 110%;margin-top:0px;margin-bottom: 0px;padding:0px;">Get our <strong>Black Friday</strong> deal and renew your LinguaPress license with our <strong>biggest sale of the year</strong>. <a class="button-primary" style="margin-top:6px;" href="https://linguapress.com/black-friday/?utm_source=tpsettings&utm_medium=clientsite&utm_campaign=BF-2025" target="_blank">Get discount</a></p></div>';
                } else {
                    ...
                    $message .= '<p style="font-size: 110%;margin-top:0px;margin-bottom: 0px;padding:0px;">Get our <strong>Black Friday</strong> deal and switch to a premium license of LinguaPress with our <strong>biggest sale of the year</strong>. <a class="button-primary" style="margin-top:6px;" href="https://linguapress.com/black-friday/?utm_source=tpsettings&utm_medium=clientsite&utm_campaign=BF-2025" target="_blank">Get discount</a></p></div>';
                }

                $message .= '<a href="' . add_query_arg( array( 'lrp_dismiss_admin_notification' => $notification_id ) ) . '" type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__( 'Dismiss this notice.', 'linguapress' ) . '</span></a>';

                $notifications->add_notification( $notification_id, $message, 'lrp-notice lrp-narrow notice notice-info lrp-bf-notice-container', true, array( 'lingua-press' ) );

            } else {

                $notification_id = 'lrp_bf_2025';

                $message = '<img style="float: left; margin: 10px 8px 10px 0px; max-width: 20px;" src="' . LRP_PLUGIN_URL . 'assets/images/tp-logo-2d.png" />';
                
                if ( !$free_version && $license_status == 'expired' )
                    $message .= '<p style="padding-right:30px;font-size: 110%;"><strong>LinguaPress Black Friday is here!</strong> Renew your <strong>PRO</strong> license with our biggest discount of the year. <a href="https://linguapress.com/black-friday/?utm_source=wpdashboard&utm_medium=clientsite&utm_campaign=BF-2025" target="_blank">Learn more</a></p>';
                else
                    $message .= '<p style="padding-right:30px;font-size: 110%;"><strong>LinguaPress Black Friday is here!</strong> Go <strong>PRO</strong> with our biggest discount of the year. <a href="https://linguapress.com/black-friday/?utm_source=wpdashboard&utm_medium=clientsite&utm_campaign=BF-2025" target="_blank">Learn more</a></p>';
...
function lrp_bf_show_promotion(){

    if( !lrp_bf_promotion_is_active() )
        return false;
...
function lrp_bf_promotion_is_active(){
