 ...
 *
 */
class LRP_Search extends WP_Query{
     ...
     * @return mixed
     */
    public function lrp_search_filter( $query ) {
        global $LRP_LANGUAGE;
        ...
        if ( $LRP_LANGUAGE !== $this->settings['default-language'] ) {
            if ( ( !is_admin() && $query->is_main_query() && $query->is_search() ) || $is_rest_request || apply_filters( 'lrp_force_search', false ) ) {
    ...
    public function get_post_ids_containing_search_term($search_query, $query = null ){
        ...
        $lrp_search_query = '';
        $dictionary_name = $this->lrp_query->get_table_name( apply_filters( 'lrp_change_search_dictionary_language', $LRP_LANGUAGE, $this, $query ) );
        $meta_table_name =  $this->lrp_query->get_table_name_for_original_meta();

        if( $search_terms_count === 1 ){
            /**
             * for one search term we can find it directly in the translated column
             */
            $lrp_search_query = $this->db->prepare( "SELECT meta_value FROM
                                        $dictionary_name
                                        INNER JOIN $meta_table_name ON $dictionary_name.original_id = $meta_table_name.original_id AND $meta_table_name.meta_key = '". $this->lrp_query->get_meta_key_for_post_parent_id() ."'
             ...
             */
            $lrp_search_query = "SELECT meta_value FROM 
        ...
        $search_result_ids = $this->db->get_results( $lrp_search_query, OBJECT_K );
     ...
     * @return string
     */
    public function lrp_search_query( $s ){
        global $LRP_LANGUAGE;
