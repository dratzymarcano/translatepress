...
class LRP_Machine_Translator {
    ...
    /**
     * LRP_Machine_Translator constructor.
     *
    ...
    public function __construct( $settings ){
        ...
        add_filter( 'lrp_exclude_words_from_automatic_translation', array( $this, 'sort_exclude_words_from_automatic_translation_array' ), 99999, 1 );
        add_filter( 'lrp_exclude_words_from_automatic_translation', array( $this, 'exclude_special_symbol_from_translation' ), 9999, 2 );

        add_filter('lrp_add_google_v2_supported_languages_to_the_array', array( $this, 'add_google_v2_supported_languages_that_are_not_returned_by_the_post_response'), 10, 1);
    }
    ...
    public function check_languages_availability( $languages, $force_recheck = false ){
        ...
        $force_recheck = ( current_user_can('manage_options') &&
            !empty( $_GET['lrp_recheck_supported_languages']) && $_GET['lrp_recheck_supported_languages'] === '1' &&
            wp_verify_nonce( sanitize_text_field( $_GET['lrp_recheck_supported_languages_nonce'] ), 'lrp_recheck_supported_languages' ) ) ? true : $force_recheck; //phpcs:ignore
        $data = get_option('lrp_db_stored_data', array() );
        if ( isset( $_GET['lrp_recheck_supported_languages'] )) {
            unset($_GET['lrp_recheck_supported_languages'] );
        }

        // if supported languages are not stored, fetch them and update option
        if ( empty( $data['lrp_mt_supported_languages'][$this->settings['lrp_machine_translation_settings']['translation-engine']]['last-checked'] ) || $force_recheck || ( method_exists($this,'check_formality') && !isset($data['lrp_mt_supported_languages'][$this->settings['lrp_machine_translation_settings']['translation-engine']]['formality-supported-languages']))){
            if ( empty( $data['lrp_mt_supported_languages'] ) ) {
                $data['lrp_mt_supported_languages'] = array();
            }
            if ( empty( $data['lrp_mt_supported_languages'][ $this->settings['lrp_machine_translation_settings']['translation-engine'] ] ) ) {
                $data['lrp_mt_supported_languages'][ $this->settings['lrp_machine_translation_settings']['translation-engine'] ] = array( 'languages' => array() );
            }

            $data['lrp_mt_supported_languages'][ $this->settings['lrp_machine_translation_settings']['translation-engine'] ]['languages'] = $this->get_supported_languages();
            if (method_exists($this, 'check_formality')) {
                $data['lrp_mt_supported_languages'][ $this->settings['lrp_machine_translation_settings']['translation-engine'] ]['formality-supported-languages'] = $this->check_formality();
            }
            $data['lrp_mt_supported_languages'][$this->settings['lrp_machine_translation_settings']['translation-engine']]['last-checked'] = date("Y-m-d H:i:s" );
            update_option('lrp_db_stored_data', $data );
        }

        $languages_iso_to_check = $this->get_engine_specific_language_codes( $languages );

        $all_are_available = !array_diff($languages_iso_to_check, $data['lrp_mt_supported_languages'][$this->settings['lrp_machine_translation_settings']['translation-engine']]['languages']);

        return apply_filters('lrp_mt_available_supported_languages', $all_are_available, $languages, $this->settings );
    }

    public function get_last_checked_supported_languages(){
        $data = get_option('lrp_db_stored_data', array() );
        if ( empty( $data['lrp_mt_supported_languages'][$this->settings['lrp_machine_translation_settings']['translation-engine']]['last-checked'] ) ){
            $this->check_languages_availability( $this->settings['translation-languages'], true);
        }
        return $data['lrp_mt_supported_languages'][$this->settings['lrp_machine_translation_settings']['translation-engine']]['last-checked'];
    }
    ...
    public function automatic_translation_svg_output( $show_errors ) {
        if ( method_exists( $this, 'automatic_translate_error_check' ) ) {
            if ( $show_errors ) {
                lrp_output_svg( 'error' );
            } else {
                lrp_output_svg( 'check' );
            }
        }
     ...
     *
     * @param LRP_Lingua_Press $machine_translator Machine translator instance.
        ...
        switch ( $translation_engine ) {
            ...
            case 'deepl':
                if ( empty( $api_key ) ) {
                    ...
                    $response = $machine_translator->test_request();
                    $code     = wp_remote_retrieve_response_code( $response );
                    if ( 200 !== $code && ( method_exists( 'LRP_DeepL', 'deepl_response_codes' ) || method_exists( 'LRP_IN_DeepL', 'deepl_response_codes' ) ) ) {

						// Test whether the old deepL add-on or the new repackaging model is used
						if ( method_exists( 'LRP_DeepL', 'deepl_response_codes' ) ) {
							$translate_response = LRP_DeepL::deepl_response_codes( $code );
						} else {
            ...
            case 'openrouter':
                if ( empty( $api_key ) ) {
                    $is_error = true;
                    $return_message = __( 'Please enter your OpenRouter API key.', 'linguapress' );
                } else {
                    // Perform test.
                    $response = $machine_translator->test_request();
                    $code     = wp_remote_retrieve_response_code( $response );
                    if ( 200 !== $code ) {
                        $is_error        = true;
                        $return_message     = __( 'There was an error accessing the OpenRouter API.', 'linguapress' );
                    }
                }
                break;
            case 'chatgpt':
                if ( empty( $api_key ) ) {
                    $is_error = true;
                    $return_message = __( 'Please enter your ChatGPT API key.', 'linguapress' );
                } else {
                    // Perform test.
                    $response = $machine_translator->test_request();
                    $code     = wp_remote_retrieve_response_code( $response );
                    if ( 200 !== $code ) {
                        $is_error        = true;
                        $return_message     = __( 'There was an error accessing the OpenAI API.', 'linguapress' );
                    }
                }
                break;
            default:
    ...
    public function verify_request( $to_language ){
        ...
        if( !empty( $this->settings['lrp_machine_translation_settings']['block-crawlers'] ) && $this->settings['lrp_machine_translation_settings']['block-crawlers'] == 'yes' && $this->is_crawler() )
            return false;
    ...
    private function is_crawler(){
        ...
        $crawlers = apply_filters( 'lrp_machine_translator_crawlers', 'rambler|abacho|acoi|accona|aspseek|altavista|estyle|scrubby|lycos|geona|ia_archiver|alexa|sogou|skype|facebook|twitter|pinterest|linkedin|naver|bing|google|yahoo|duckduckgo|yandex|baidu|teoma|xing|java\/1.7.0_45|bot|crawl|slurp|spider|mediapartners|\sask\s|\saol\s' );
    ...
    public function translate($strings, $target_language_code, $source_language_code = null ){
        if ( !empty($strings) && is_array($strings) && method_exists( $this, 'translate_array' ) && apply_filters( 'lrp_disable_automatic_translations_due_to_error', false ) === false ) {
            ...
            $lrp_exclude_words_from_automatic_translation = apply_filters('lrp_exclude_words_from_automatic_translation', array('%s', '%d', '%', '$', '#'), $imploded_strings);
            $placeholders = $this->get_placeholders(count($lrp_exclude_words_from_automatic_translation));
            $shortcode_tags_to_execute = apply_filters( 'lrp_do_these_shortcodes_before_automatic_translation', array('lrp_language') );
            ...
            foreach ($strings as $key => $string) {
                ...
                $strings[$key] = str_replace($lrp_exclude_words_from_automatic_translation, $placeholders, html_entity_decode( $string ));
                $strings[$key] = lrp_do_these_shortcodes( $strings[$key], $shortcode_tags_to_execute );
            }

            if ( $this->settings['lrp_machine_translation_settings']['translation-engine'] === 'deepl' && defined( 'LRP_DL_PLUGIN_VERSION' ) && LRP_DL_PLUGIN_VERSION === '1.0.0' ) {
            ...
            if (!empty($machine_strings)) {
                foreach ($machine_strings as $key => $machine_string) {
                    $machine_strings_return_array[$original_strings[$key]] = str_ireplace( $placeholders, $lrp_exclude_words_from_automatic_translation, $machine_string );
                }
            }
    ...
    /**
     * @param $lrp_exclude_words_from_automatic_translation
     * @return mixed
     *
     * We need to sort the $lrp_exclude_words_from_automatic_translation array descending because we risk to not translate excluded multiple words when one
     ...
     */
    public function sort_exclude_words_from_automatic_translation_array($lrp_exclude_words_from_automatic_translation){
        usort($lrp_exclude_words_from_automatic_translation, array($this,"sort_array"));

        return $lrp_exclude_words_from_automatic_translation;
    }
    ...
    public function get_api_key(){
        ...
        if( !empty( $this->settings['lrp_machine_translation_settings']['translation-engine'] ) ){
            switch( $this->settings['lrp_machine_translation_settings']['translation-engine'] ){
                case 'google_translate_v2':
                    if( !empty( $this->settings['lrp_machine_translation_settings']['google-translate-key'] ) )
                        $key = $this->settings['lrp_machine_translation_settings']['google-translate-key'];
                    break;
                case 'deepl':
                    if( !empty( $this->settings['lrp_machine_translation_settings']['deepl-api-key'] ) )
                        $key = $this->settings['lrp_machine_translation_settings']['deepl-api-key'];
                    break;
                case 'openrouter':
                    if( !empty( $this->settings['lrp_machine_translation_settings']['openrouter-api-key'] ) )
                        $key = $this->settings['lrp_machine_translation_settings']['openrouter-api-key'];
                    break;
                case 'chatgpt':
                    if( !empty( $this->settings['lrp_machine_translation_settings']['chatgpt-api-key'] ) )
                        $key = $this->settings['lrp_machine_translation_settings']['chatgpt-api-key'];
                    break;
            }
        }
        return $key;
