...
class LRP_Editor_Api_Gettext_Strings {

	/* @var LRP_Query */
	protected $lrp_query;
	/* @var LRP_SP_Slug_Manager*/
	...
	/**
	 * Hooked to wp_ajax_lrp_get_translations_gettext
	 */
	public function gettext_get_translations() {
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			if ( isset( $_POST['action'] ) && $_POST['action'] === 'lrp_get_translations_gettext' && ! empty( $_POST['string_ids'] ) && ! empty( $_POST['language'] ) && in_array( $_POST['language'], $this->settings['translation-languages'] ) ) {
				...
				if ( is_array( $gettext_string_ids ) ) {
					...
					$dictionaries[ $current_language ] = $this->lrp_query->get_gettext_string_rows_by_ids( $gettext_string_ids, $current_language );
	...
	public function gettext_save_translations(){
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX && current_user_can( apply_filters( 'lrp_translating_capability', 'manage_options' ) ) ) {
			if (isset($_POST['action']) && $_POST['action'] === 'lrp_save_translations_gettext' && !empty($_POST['strings'])) {
				...
				foreach( $update_strings as $language => $update_string_array ) {
                    ...
                    $this->lrp_query->remove_possible_duplicates($update_string_array, $language, 'gettext');
				}

                do_action('lrp_save_editor_translations_gettext_strings', $update_strings, $this->settings);
			}
		}
