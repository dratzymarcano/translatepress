...
class LRP_Ajax{
    ...
    /**
     * LRP_Ajax constructor.
     *
     * Establishes db connection and triggers function to output translations.
     */
    public function __construct( ){

        if ( !isset( $_POST['action'] ) || $_POST['action'] !== 'lrp_get_translations_regular' || empty( $_POST['originals'] ) || empty( $_POST['language'] ) || empty( $_POST['original_language'] ) ) {
            die();
        }

        include './external-functions.php';
        if ( !lrp_is_valid_language_code( $_POST['language'] ) || !lrp_is_valid_language_code( $_POST['original_language'] ) ) {//phpcs:ignore
            echo json_encode( 'LinguaPress Error: Invalid language code' );
            exit;
        }
        ...
        if ( $this->connect_to_db() ){
            ...
            $this->output_translations(
	            ...
	            mysqli_real_escape_string( $this->connection, $_POST['language'] ), /* phpcs:ignore */ /* validated with lrp_is_valid_language_code on line 25 */
	            mysqli_real_escape_string( $this->connection, $_POST['original_language'] ) /* phpcs:ignore */ /* validated with lrp_is_valid_language_code on line 25 */
            );
    ...
    protected function sanitize_strings( $posted_strings){
        ...
        if ( is_array( $strings ) ) {
            foreach ($strings as $key => $string) {
	            $strings[$key] = mysqli_real_escape_string( $this->connection, lrp_full_trim( $string, array( 'numerals'=> $numerals_option ) ) );
            }
        }
        return $strings;
    }
    ...
    protected function output_translations( $strings, $skip_machine_translation, $language, $original_language ){
        $sql = 'SELECT original, translated, status FROM ' . $this->table_prefix . 'lrp_dictionary_' . strtolower( $original_language ) . '_' . strtolower( $language ) . ' WHERE original IN (\'' . implode( "','", $strings ) .'\') AND status != 0';
        try {
            $result = mysqli_query( $this->connection, $sql );
        }catch(Throwable $e){
            $this->return_error();
        ...
        if ( $result === false ){
	        ...
	        $dictionary_by_original = lrp_sort_dictionary_by_original( $dictionaries, 'regular', 'dynamicstrings', null, null );
