 ...
 * @return bool
 */
function lrp_beaver_builder_compatibility( $url ){
...
add_filter( 'lrp_edit_translation_url', 'lrp_beaver_builder_compatibility' );


/**
 * Mb Strings missing PHP library error notice
 */
function lrp_mbstrings_notification(){
    echo '<div class="notice notice-error"><p>' . wp_kses( __( '<strong>LinguaPress</strong> requires <strong><a href="http://php.net/manual/en/book.mbstring.php">Multibyte String PHP library</a></strong>. Please contact your server administrator to install it on your server.','linguapress' ), [ 'a' => [ 'href' => [] ], 'strong' => [] ] ) . '</p></div>';
}

function lrp_missing_mbstrings_library( $allow_to_run ){
    if ( ! extension_loaded('mbstring') ) {
        add_action( 'admin_menu', 'lrp_mbstrings_notification' );
        return false;
    }
    return $allow_to_run;
}
add_filter( 'lrp_allow_tp_to_run', 'lrp_missing_mbstrings_library' );

/**
 * Don't have html inside menu title tags. Some themes just put in the title the content of the link without striping HTML
 */
add_filter( 'nav_menu_link_attributes', 'lrp_remove_html_from_menu_title', 10, 3);
function lrp_remove_html_from_menu_title( $atts, $item, $args ){
    if( isset( $atts['title'] ) )
 ...
 */
function lrp_wp_trim_words( $text, $num_words, $more, $original_text ) {
    if ( null === $more ) {
...
add_filter('wp_trim_words', 'lrp_wp_trim_words', 100, 4);
 ...
 * @return array
 */
add_filter( 'peepso_data', 'lrp_use_home_url_in_peepso_ajax' );
function lrp_use_home_url_in_peepso_ajax( $data ){
 ...
 */
add_filter('peepso_get_page', 'lrp_remove_peepso_double_slash', 10, 2);
function lrp_remove_peepso_double_slash( $page, $name){
...
add_filter('ginger_iframe_banner', 'lrp_do_shortcode', 999 );
add_filter('ginger_text_banner', 'lrp_do_shortcode', 999 );
function lrp_do_shortcode($content){
...
add_filter( 'lrp_skip_gettext_processing', 'lrp_woo_strip_gettext_from_print_products' );

function lrp_woo_strip_gettext_from_print_products( $bool ){
...
add_filter('lrp_stop_translating_page', 'lrp_woo_pdf_print_products', 10, 2);

function lrp_woo_pdf_print_products( $bool, $output ){
    if ( isset( $_REQUEST['print-products'] ) && $_REQUEST['print-products'] == 'pdf' && class_exists('\WooCommerce_Print_Products') ) {
        return true;
    }
    return $bool;
 ...
 *
 * The DK PDF plugin seems to not work at all. Even when LinguaPress is deactivated, there are critical errors and notices in debug.log
 */

add_filter( 'lrp_skip_gettext_processing', 'lrp_dk_pdf_strip_gettext_from_pdf' );

function lrp_dk_pdf_strip_gettext_from_pdf( $bool ){
...
add_filter('lrp_stop_translating_page', 'lrp_do_not_translate_dk_pdf', 10, 2);
function lrp_do_not_translate_dk_pdf($translate, $output){
...
add_filter( 'lrp_skip_gettext_processing', 'lrp_invoices_for_woocommerce_strip_gettext_from_pdf', 10, 4 );
function lrp_invoices_for_woocommerce_strip_gettext_from_pdf( $bool, $translation, $text, $domain ){
...
add_filter('lrp_stop_translating_page', 'lrp_do_not_translate_pdf_param', 10, 2);
function lrp_do_not_translate_pdf_param($translate, $output){
...
add_filter( 'wpo_wcpdf_filename', 'lrp_woo_pdf_invoices_and_packing_slips_compatibility' );

// fix #lrpgettext inside invoice pdf
add_filter( 'wpo_wcpdf_get_html', 'lrp_woo_pdf_invoices_and_packing_slips_compatibility');
function lrp_woo_pdf_invoices_and_packing_slips_compatibility($title){
    if ( class_exists( 'LRP_Translation_Manager' ) ) {
        return 	LRP_Translation_Manager::strip_gettext_tags($title);
    }
}

// fix font of pdf breaking because of str_get_html() call inside translate_page()
add_filter( 'lrp_stop_translating_page', 'lrp_woo_pdf_invoices_and_packing_slips_compatibility_dont_translate_pdf', 10, 2 );
function lrp_woo_pdf_invoices_and_packing_slips_compatibility_dont_translate_pdf( $bool, $output ){
 ...
 *
 */
add_filter( 'woocommerce_pdf_invoices_content', 'lrp_woo_ultimate_pdf_invoices_compatibility');
add_filter( 'woocommerce_pdf_invoices_order_data', 'lrp_woo_ultimate_pdf_invoices_data_compatibility');

function lrp_woo_ultimate_pdf_invoices_compatibility($title){
    if ( class_exists( 'LRP_Translation_Manager' ) ) {
        return 	LRP_Translation_Manager::strip_gettext_tags($title);
    }
}

function lrp_woo_ultimate_pdf_invoices_data_compatibility($data_array){
    if ( class_exists( 'LRP_Translation_Manager' ) ) {
        $data_array = array_map('LRP_Translation_Manager::strip_gettext_tags',$data_array );
    }
    return $data_array;
}
 ...
 *
 */
add_filter( 'lrp_stop_translating_page', 'lrp_woocommerce_pdf_catalog_compatibility_dont_translate_pdf', 10, 2 );
function lrp_woocommerce_pdf_catalog_compatibility_dont_translate_pdf( $bool, $output ){
...
add_filter( 'lrp_skip_gettext_processing', 'lrp_woo_strip_gettext_from_yith_pdf', 10, 4 );
function lrp_woo_strip_gettext_from_yith_pdf( $bool, $translation, $text, $domain ){
...
add_filter( 'lrp_stop_translating_page', 'lrp_woo_pdf_invoices_compatibility_dont_translate_pdf', 10, 2 );
function lrp_woo_pdf_invoices_compatibility_dont_translate_pdf( $bool, $output ){
 ...
 *
 * When a new order is placed in secondary languages, in admin area WooCommerce->Orders->Edit Order, the right sidebar contains Order notes which can contain #lrpst tags.
 *
 * @since 1.4.3
 */

// old orders
add_filter( 'woocommerce_get_order_note', 'lrp_woo_notes_strip_lrpst' );
// new orders
add_filter( 'woocommerce_new_order_note_data', 'lrp_woo_notes_strip_lrpst' );
function lrp_woo_notes_strip_lrpst( $note_array ){
    foreach ( $note_array as $item => $value ){
        $note_array[$item] = LRP_Translation_Manager::strip_gettext_tags( $value );
    }
    return $note_array;
}
 ...
 */
add_filter('woocommerce_order_item_display_meta_key','lrp_woo_data_strip_lrpst');
add_filter('woocommerce_order_item_get_method_title','lrp_woo_data_strip_lrpst');
function lrp_woo_data_strip_lrpst( $data ){
    return LRP_Translation_Manager::strip_gettext_tags( $data );
}
 ...
 *
 */
add_filter( 'lrp_skip_selectors_from_dynamic_translation', 'lrp_woo_skip_dynamic_translation' );
function lrp_woo_skip_dynamic_translation( $skip_selectors ){
 ...
 */
add_action( 'woocommerce_email_customer_details', 'lrp_woo_prevent_address_from_translation_in_emails' );
function lrp_woo_prevent_address_from_translation_in_emails(){
    add_filter( 'woocommerce_order_get_formatted_shipping_address', 'lrp_woo_address_no_translate', 10, 3 );
    add_filter( 'woocommerce_order_get_formatted_billing_address', 'lrp_woo_address_no_translate', 10, 3 );
}

function lrp_woo_address_no_translate( $address, $raw_address, $order ){
 ...
 */
add_filter( 'woocommerce_product_variation_title', 'lrp_woo_wrap_variation', 8, 4);
function lrp_woo_wrap_variation($name, $product, $title_base, $title_suffix){
    $separator  = '<span> - </span>';
    return $title_suffix ? $title_base . $separator . $title_suffix : $title_base;
}

// lrpgettext tags don't get escaped because they add <small> tags through a regex.
add_filter( 'qm/output/title', 'lrp_qm_strip_gettext', 100);
function lrp_qm_strip_gettext( $data ){
    if ( is_array( $data ) ) {
        foreach( $data as $key => $value ){
            $data[$key] = lrp_qm_strip_gettext($value);
        }
    }else {
        // remove small tags
        $data = preg_replace('(<(\/)?small>)', '', $data);
        // strip gettext (not needed, they are just numbers shown in admin bar anyway)
        $data = LRP_Translation_Manager::strip_gettext_tags( $data );
 ...
 */
add_filter( 'lrp_translated_html', 'lrp_force_include_scripts', 10, 4 );
function lrp_force_include_scripts( $final_html, $LRP_LANGUAGE, $language_code, $preview_mode ){
    if ( $preview_mode ){
        $lrp = LRP_Lingua_Press::get_lrp_instance();
        $translation_render = $lrp->get_component( 'translation_render' );
        $lrp_data = $translation_render->get_lrp_data();

        $scripts_and_styles = apply_filters('lrp_editor_missing_scripts_and_styles', array(
            'jquery'                        => "<script type='text/javascript' src='" . includes_url( '/js/jquery/jquery.js' ) . "'></script>",
            'lrp-iframe-preview-script.js'  => "<script type='text/javascript' src='" . LRP_PLUGIN_URL . "assets/js/lrp-iframe-preview-script.js'></script>",
            'lrp-translate-dom-changes.js'  => "<script>lrp_data = '" . addslashes(json_encode($lrp_data) ) . "'; lrp_data = JSON.parse(lrp_data);</script><script type='text/javascript' src='" . LRP_PLUGIN_URL . "assets/js/lrp-translate-dom-changes.js'></script>",
            'lrp-preview-iframe-style-css'  => "<link rel='stylesheet' id='lrp-preview-iframe-style-css'  href='" . LRP_PLUGIN_URL . "assets/css/lrp-preview-iframe-style.css' type='text/css' media='all' />",
        ...
        if ( $missing_script !== '' ){
            $html = LinguaPress\str_get_html( $final_html, true, true, LRP_DEFAULT_TARGET_CHARSET, false, LRP_DEFAULT_BR_TEXT, LRP_DEFAULT_SPAN_TEXT );
            if ( $html === false ) {
                return $final_html;
            }
...
add_filter( 'http_request_args', 'lrp_strip_lrpst_from_requests', 10, 2 );
function lrp_strip_lrpst_from_requests($args, $url){
    if( is_array( $args['body'] ) ) {
        array_walk_recursive( $args['body'], 'lrp_array_walk_recursive_strip_gettext_tags' );
    }else{
        $args['body'] = LRP_Translation_Manager::strip_gettext_tags( $args['body'] );
    }
    return $args;
}
function lrp_array_walk_recursive_strip_gettext_tags( &$value ){
    $value = LRP_Translation_Manager::strip_gettext_tags( $value );
}

// Strip of gettext wrappings the customer_name and customer_email keys. Found in WC Stripe and Cartflows
add_filter( 'wc_stripe_payment_metadata', 'lrp_strip_request_metadata_keys' );
function lrp_strip_request_metadata_keys( $metadata ){
    foreach( $metadata as $key => $value ) {
        $stripped_key = LRP_Translation_Manager::strip_gettext_tags( $key );
        if ( $stripped_key != $key ) {
 ...
 * resulting in outputting scripts that won't be stripped of lrpst lrp-gettext wrappings.
 * This includes WooCommerce Checkout scripts, resulting in lrpst wrappings around form fields like Street Address.
 ...
 */
add_filter( 'lrp_start_output_buffer_priority', 'lrp_nextgen_compatibility' );
function lrp_nextgen_compatibility( $priority ){
 ...
 */
add_filter( 'run_ngg_resource_manager', 'lrp_nextgen_disable_nextgen_in_translation_editor');
function lrp_nextgen_disable_nextgen_in_translation_editor( $bool ){
    if ( isset( $_REQUEST['lrp-edit-translation'] ) && sanitize_text_field( $_REQUEST['lrp-edit-translation'] ) === 'true' ) {
        return false;
    }
    return $bool;
}
 ...
 *
 */
add_filter( 'the_title', 'lrp_woo_translate_product_title_added_to_cart', 10, 2 );
function lrp_woo_translate_product_title_added_to_cart( ...$args ){
    ...
    if( class_exists( 'WooCommerce' ) ){
        ...
        $list_of_functions = apply_filters( 'lrp_woo_translate_title_before_translate_page', array( 'wc_add_to_cart_message' ) );
...
if( class_exists( 'WooCommerce' ) ) {
	add_filter( 'woocommerce_cart_item_removed_title', 'lrp_woo_fix_product_remove_from_cart_notice', 10, 2 );

	function lrp_woo_fix_product_remove_from_cart_notice($message, $cart_item){
 ...
 *
 * They replace spaces (" ") with \u0020, after we apply #lrpst and because we don't strip them it breaks html
 */
add_action('init', 'lrp_wootour_add_gettext_filter');
function lrp_wootour_add_gettext_filter(){
    if ( class_exists( 'WooTour_Booking' ) ){
        add_filter('gettext', 'lrp_wootour_exclude_gettext_strings', 1000, 3 );
    }
}
function lrp_wootour_exclude_gettext_strings($translation, $text, $domain){
    if ( $domain == 'woo-tour' ){
        if ( in_array( $text, array( 'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December' ) ) ){
            return LRP_Translation_Manager::strip_gettext_tags( $translation );
        }
    }
    return $translation;
}
 ...
 */
add_filter( 'woocommerce_cart_item_name', 'lrp_woo_cart_item_name', 8, 3 );
function lrp_woo_cart_item_name( $product_name, $cart_item, $cart_item_key ){
 ...
 */
add_filter( 'wpo_wcpdf_order_item_data', 'lrp_woo_wcpdf_translate_product_name', 10, 3 );
function lrp_woo_wcpdf_translate_product_name( $data, $order, $type ){
    if ( isset( $data['name'] ) ) {
        $lrp = LRP_Lingua_Press::get_lrp_instance();
        $translation_render = $lrp->get_component('translation_render');
        remove_filter( 'lrp_stop_translating_page', 'lrp_woo_pdf_invoices_and_packing_slips_compatibility_dont_translate_pdf', 10 );
 ...
 */
add_action( 'woocommerce_cart_calculate_fees', 'lrp_woo_checkout_add_ons_filter_lrpstr', 10, 2);
function lrp_woo_checkout_add_ons_filter_lrpstr(){
    if ( class_exists('WC_Checkout_Add_Ons_Frontend') ) {
        add_filter('lrp_skip_gettext_processing', 'lrp_woo_checkout_exclude_strings', 1000, 4);
    }
}

function lrp_woo_checkout_exclude_strings( $return, $translation, $text, $domain) {
 ...
 */
add_action('init', 'lrp_woo_fondy_payment_gateway_add_gettext_filter');
function lrp_woo_fondy_payment_gateway_add_gettext_filter(){
    if ( class_exists( 'WC_fondy' ) ){
        add_filter('gettext', 'lrp_woo_fondy_payment_gateway_exclude_gettext_strings', 1000, 3 );
    }
}

function lrp_woo_fondy_payment_gateway_exclude_gettext_strings($translation, $text, $domain){
 ...
 *
 * They stop the buffering at priority -150 and that leaves #lrpst style tags before we get to remove them
 *
...
add_action( 'init', 'lrp_woo_product_filters', 10 );

function lrp_woo_product_filters(){
...
if ( class_exists( 'Barn2\Plugin\WC_Filters\Plugin_Factory' ) ) {
    add_filter( "lrp_output_buffer_chunk_size", "lrp_set_chunk_size_to_zero", 10, 1 );
}
function lrp_set_chunk_size_to_zero( $chunk_size ) {
    $chunk_size = 0;
 ...
 *
 */
add_filter('lrp_skip_url_for_language', 'lrp_skip_elementor_popup_action_from_url_converter', 10, 2);
function lrp_skip_elementor_popup_action_from_url_converter($value, $url){
 ...
 */
add_filter('get_the_date','lrp_strip_gettext_from_get_the_date', 1, 3);
function lrp_strip_gettext_from_get_the_date($the_date, $d = NULL, $post = NULL){
	if ( $d === NULL || $post === NULL ){
		return $the_date;
	}

    $d = LRP_Translation_Manager::strip_gettext_tags( $d );
    $post = get_post( $post );
 ...
 * LinguaPress adds the lrp-form-language for other reasons. So we need to remove it in this case.
 * https://affiliatetheme.io
 *
 */
add_filter('at_set_product_filter_query', 'lrp_remove_lang_param_from_query');
function lrp_remove_lang_param_from_query($args){

	if ( isset( $args['meta_query'] ) && is_array( $args['meta_query']) ){
		foreach($args['meta_query'] as $key => $value){
			if ($value['key'] == 'lrp-form-language'){
 ...
 *
 */
add_action( 'user_register', 'lrp_add_user_prefered_language', 10 );
function lrp_add_user_prefered_language($user_id) {
	global $LRP_LANGUAGE;
	if ( ! empty( $LRP_LANGUAGE ) ) {
		$user_locale = get_user_meta( $user_id, 'locale', true );
		if ( empty( $user_locale ) ) {
			update_user_meta( $user_id, 'locale', $LRP_LANGUAGE );
		}
	}
}
 ...
 */
add_action('wp_enqueue_scripts', 'lrp_remove_dflip_defer_script', 9999);
function lrp_remove_dflip_defer_script(){
 ...
 * This will insert lrpstr strings in the database. So just ignore it.
 *
 */
add_filter('lrp_skip_gettext_processing', 'lrp_exclude_woo_display_name_gettext', 2000, 4 );
function lrp_exclude_woo_display_name_gettext ( $return, $translation, $text, $domain ){
 ...
 */
add_action('wp_head','lrp_superfly_change_menu_loading_hook', 5);
function lrp_superfly_change_menu_loading_hook(){
 ...
 */
add_filter( 'wpseo_canonical', 'lrp_wpseo_canonical_compat', 99999, 2);
function lrp_wpseo_canonical_compat( $canonical, $presentation_class = null ){
    global $LRP_LANGUAGE;
    $lrp           = LRP_Lingua_Press::get_lrp_instance();
    $url_converter = $lrp->get_component( 'url_converter' );
    $canonical     = $url_converter->get_url_for_language( $LRP_LANGUAGE, $canonical, '' );

    return $canonical;
};

add_filter( 'wpseo_opengraph_url', 'lrp_opengraph_url', 99999 );
function lrp_opengraph_url( $url ) {
	global $LRP_LANGUAGE;
	$lrp = LRP_Lingua_Press::get_lrp_instance();
	$url_converter = $lrp->get_component( 'url_converter' );
	$url = $url_converter->get_url_for_language($LRP_LANGUAGE, $url, '');
 ...
 */
add_action( 'lrp_before_running_hooks', 'lrp_oxygen_remove_gettext_hooks', 10, 1 );
function lrp_oxygen_remove_gettext_hooks( $lrp_loader ) {
    if ( isset( $_REQUEST['xlink'] ) && $_REQUEST['xlink'] === 'css' ) {
        ...
        $lrp_loader->remove_hook( 'init', 'initialize_gettext_processing', $gettext_manager );
        $lrp_loader->remove_hook( 'shutdown', 'machine_translate_gettext', $gettext_manager );
        $lrp_loader->remove_hook( 'init', 'start_output_buffer', $translation_render );
...
if( function_exists('ct_is_show_builder') ) {
    ...
    function tp_oxygen_search_compatibility($output, $tag, $attr, $m){
    	...
    	if( $tag === 'ct_headline' || $tag === 'ct_text_block' ) {
            ...
            if ($LRP_LANGUAGE !== $settings['default-language']) {
                if (is_singular() && !empty($post->ID)) {
                    $output = "<lrp-post-container data-lrp-post-id='" . $post->ID . "'>" . $output . "</lrp-post-container>";//changed " to ' to not break cases when the filter is applied inside an html attribute (title for example)
                }
            }
        }
    ...
    /**
     * Disable LRP when the Oxygen Builder is being loaded
     */
    add_filter( 'lrp_stop_translating_page', 'lrp_oxygen_disable_lrp_in_builder', 10, 2);
    function lrp_oxygen_disable_lrp_in_builder($bool, $output){
     ...
     */
    add_filter( 'lrp_floating_ls_html', 'lrp_page_builder_compatibility_disable_language_switcher' );
    function lrp_page_builder_compatibility_disable_language_switcher( $html ){
...
if( function_exists( 'ct_is_show_builder' ) || defined( 'FL_BUILDER_VERSION' ) ){
     ...
     * Hooked before LRP_Language_Switcher::redirect_to_correct_language() so we don't redirect twice
     */
    add_action( 'template_redirect', 'lrp_page_builder_compatibility_redirect_to_default_language', 10 );
    function lrp_page_builder_compatibility_redirect_to_default_language(){

        if( !is_admin() && ( ( isset( $_GET['ct_builder'] ) && $_GET['ct_builder'] == 'true' ) || isset( $_GET['fl_builder'] ) ) ){

            $lrp           = LRP_Lingua_Press::get_lrp_instance();
            $url_converter = $lrp->get_component('url_converter');
            $settings      = ( new LRP_Settings() )->get_settings();
     ...
     */
    add_filter( 'lrp_ald_enqueue_redirecting_script', 'lrp_ald_dont_redirect_inside_page_builders');
    function lrp_ald_dont_redirect_inside_page_builders( $enqueue_redirecting_script ){
 ...
 */
add_filter( 'lrp_enable_dynamic_translation', 'lrp_brizy_disable_dynamic_translation' );
function lrp_brizy_disable_dynamic_translation( $enable ){
...
if( defined( 'BRIZY_PRO_VERSION' ) || defined( 'BRIZY_VERSION' ) ){
    add_filter( 'lrp_home_url', 'lrp_brizy_menu_pro_compatibility', 10, 5 );
    function lrp_brizy_menu_pro_compatibility( $new_url, $abs_home, $LRP_LANGUAGE, $path, $url ){
        if ( version_compare( PHP_VERSION, '5.4.0', '>=' ) ) {
            $callstack_functions = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 15);//set a limit if it is supported to improve performance
        }
        else{
...
if( defined( 'WOO_VOU_PLUGIN_VERSION' ) ){
    add_filter( 'lrp_home_url', 'lrp_woocommerce_pdf_vouchers_download_file_compatibility', 10, 5 );
    function lrp_woocommerce_pdf_vouchers_download_file_compatibility( $new_url, $abs_home, $LRP_LANGUAGE, $path, $url ){
        if ( version_compare( PHP_VERSION, '5.4.0', '>=' ) ) {
            $callstack_functions = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 15);//set a limit if it is supported to improve performance
        }
        else{
...
add_filter( 'aws_search_results_products_ids', 'lrp_aws_search_results_products_ids', 10, 2 );
function lrp_aws_search_results_products_ids(  $posts_ids, $s ){
    global $LRP_LANGUAGE;
    $lrp = LRP_Lingua_Press::get_lrp_instance();
    $lrp_settings = $lrp->get_component( 'settings' );
    $settings = $lrp_settings->get_settings();
    ...
    if ( $LRP_LANGUAGE !== $settings['default-language'] ) {
        $lrp_search = $lrp->get_component( 'search' );
        $search_result_ids = $lrp_search->get_post_ids_containing_search_term($s, null);
 ...
 */
add_filter( 'wpml_current_language', 'lrp_aws_current_language' );
function lrp_aws_current_language( $lang ) {
 ...
 */
add_filter('tcb_allow_landing_page_edit', 'lrp_thrive_arhitect_compatibility');
add_filter('tcb_is_editor_page', 'lrp_thrive_arhitect_compatibility');//this is for Thrive theme
function lrp_thrive_arhitect_compatibility($bool)    {
...
add_filter( 'lrp_allow_language_redirect', 'lrp_thrive_no_redirect_in_editor', 10, 3 );
function lrp_thrive_no_redirect_in_editor( $allow_redirect, $needed_language, $current_page_url ){
...
add_filter('lrp_skip_url_for_language', 'lrp_thrive_skip_language_in_editor', 10, 2);
function lrp_thrive_skip_language_in_editor($skip, $url){
...
/**
 * Compatibility with the RECON gateway for woocommerce. We must not send the "lrp-form-language" hidden field in the post request to the gateway
 */
if( class_exists('WC_Gateway_RECON') ) {
    add_filter('lrp_form_inputs', 'lrp_recon_gateway_compatibility', 10, 4);
    function lrp_recon_gateway_compatibility($input, $lrp_language, $slug, $row)
    {
...
add_filter( 'rtcl_inline_search_autocomplete_args', 'lrp_rtcl_autocomplete_search_results', 10, 2 );
function lrp_rtcl_autocomplete_search_results( $args, $request ){
    global $LRP_LANGUAGE;
    $lrp = LRP_Lingua_Press::get_lrp_instance();
    $lrp_settings = $lrp->get_component( 'settings' );
    $settings = $lrp_settings->get_settings();
...
// Otherwise lrp-post-container is not added
add_action( 'wp_body_open', 'lrp_overrule_main_query_condition', 10, 2 );
function lrp_overrule_main_query_condition(){
    if ( class_exists('Rtcl') ) {
        add_filter( 'lrp_wrap_with_post_id_overrule', '__return_false' );
    }
}

/**
 * Otherwise lrp-post-container is stripped
 *
 * Applied this solution permanently. It's problematic with Elementor and WooCommerce too.
 */
add_filter( 'wp_kses_allowed_html', 'lrp_prevent_kses_from_stripping_lrp_post_container', 10, 2 );
function lrp_prevent_kses_from_stripping_lrp_post_container( $allowedposttags, $context  ) {
    if ( $context === 'post' ){
        $allowedposttags['lrp-post-container'] = array( 'data-lrp-post-id' => true );
    }
    return $allowedposttags;
}

// Filter search results to show secondary language results
add_action('rtcl_listing_query', 'lrp_rtcl_search_results', 10, 2);
function lrp_rtcl_search_results ($q, $t){
    if ( empty( $q->get('s')) ){
        return;
    }
    global $LRP_LANGUAGE;
    ...
    if ( $LRP_LANGUAGE !== $settings['default-language'] ) {
        $lrp_search = $lrp->get_component( 'search' );
        $search_result_ids = $lrp_search->get_post_ids_containing_search_term($q->get('s'), null);
        $q->set('s', '');
        if ( empty($search_result_ids)){
 ...
 *
 */
add_filter( 'lrp_skip_selectors_from_dynamic_translation', 'lrp_add_to_any_skip_dynamic_translation' );
function lrp_add_to_any_skip_dynamic_translation( $skip_selectors ){
 ...
 *
 */
add_filter( 'lrp_skip_selectors_from_dynamic_translation', 'lrp_uncode_skip_dynamic_translation' );
function lrp_uncode_skip_dynamic_translation( $skip_selectors ){
...
add_filter( 'lrp_home_url', 'lrp_skip_home_url_processing_for_pdfemb_server_url', 10, 5 );
function lrp_skip_home_url_processing_for_pdfemb_server_url(  $new_url, $abs_home, $LRP_LANGUAGE, $path, $url ){
...
add_filter( 'lrp_skip_url_for_language', 'lrp_skip_link_processing_for_pdfemb_server_url', 10, 2 );
function lrp_skip_link_processing_for_pdfemb_server_url( $skip, $url ){
 ...
 */
add_filter( 'wp_parse_str', 'lrp_elementor_blockquote_translate_tweet_button' );
function lrp_elementor_blockquote_translate_tweet_button( $array ){
...
if( function_exists('elementor_pro_load_plugin') ) {
    add_filter('lrp_home_url', 'lrp_elementor_blockquote_tweet_button_url', 10, 5);
    function lrp_elementor_blockquote_tweet_button_url($new_url, $abs_home, $LRP_LANGUAGE, $path, $url)
    {
        if (version_compare(PHP_VERSION, '5.4.0', '>=')) {
            $callstack_functions = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 15);//set a limit if it is supported to improve performance
        } else {
...
if( defined('ELEMENTOR_VERSION') ) {
    add_filter('the_post', 'lrp_elementor_translate_tweet_button_excerpt');
    function lrp_elementor_translate_tweet_button_excerpt($post){
 ...
 */
add_filter('wp_nav_menu_objects', 'lrp_add_current_menu_item_css_class');
function lrp_add_current_menu_item_css_class( $items ){
    global $LRP_LANGUAGE;
    $lrp = LRP_Lingua_Press::get_lrp_instance();
    $url_converter = $lrp->get_component('url_converter');
    $lrp_settings = $lrp->get_component( 'settings' );
    ...
    add_filter('pre_get_posts', 'lrp_the_event_calendar_set_query_to_true', 2, 1);

    foreach( $items as $item ){
        if ( !( $LRP_LANGUAGE === $settings['default-language'] && isset( $settings['add-subdirectory-to-default-language']) && $settings['add-subdirectory-to-default-language'] !== 'yes'  ) &&
            !in_array( 'current-menu-item', $item->classes ) && !in_array( 'menu-item-object-language_switcher', $item->classes ) && ( !empty($item->url) && $item->url !== '#')
        ){
            $url_for_language = $url_converter->get_url_for_language( $LRP_LANGUAGE, $item->url );
        ...
        if(!in_array('current-language-menu-item', $item->classes) && in_array('menu-item-object-language_switcher', $item->classes)){
            ...
            if($current_language == $LRP_LANGUAGE){
                $item->classes[] = 'current-language-menu-item';
            }
        }
    }

    remove_filter('pre_get_posts', 'lrp_the_event_calendar_set_query_to_true', 2);
    return $items;
}
 ...
 * @return mixed
 */
function lrp_the_event_calendar_set_query_to_true($query){
...
if( function_exists('initial_ETC') ) {
    add_filter('lrp_skip_gettext_processing', 'lrp_exclude_xstore_search_class', 999, 4);
    function lrp_exclude_xstore_search_class($return, $translation, $text, $domain){
...
if( defined( 'WPBDP_PLUGIN_FILE' ) ) {
    add_filter('lrp_prevent_permalink_update_on_other_languages', 'lrp_prevent_permalink_update_on_other_languages');
    function lrp_prevent_permalink_update_on_other_languages($bool){
        return false;
    }
}
 ...
 */
add_filter('lrp_skip_gettext_processing', 'lrp_exclude_problematic_gettext_strings', 999, 4 );
function lrp_exclude_problematic_gettext_strings ( $return, $translation, $text, $domain ){
 ...
 * Paypal had lrpst in the details sent.
 */
add_action( 'woocommerce_api_request', 'lrp_woo_wc_api_handle_api_request', 1 );
function lrp_woo_wc_api_handle_api_request( ){
    add_filter( 'lrp_skip_gettext_processing', '__return_true' );
    add_filter( 'lrp_stop_translating_page', '__return_true' );
}
...
if( class_exists('WC_Min_Max_Quantities') ) {
    add_filter('woocommerce_loop_add_to_cart_link', 'lrp_check_duplicate_quantity_attribute_on_link', 99, 2);
    function lrp_check_duplicate_quantity_attribute_on_link($html, $product){
 ...
 */
add_filter('lrp_force_search', 'lrp_force_search' );
function lrp_force_search( $bool ){
...
if( class_exists('WooCommerce_Product_Search_Service') ) {
    ...
    add_filter('woocommerce_product_search_request_search_query', 'lrp_woocommerce_product_search_compatibility');

    function lrp_woocommerce_product_search_compatibility($search_query)
    {
        global $LRP_LANGUAGE;
        $lrp = LRP_Lingua_Press::get_lrp_instance();
        $lrp_settings = $lrp->get_component('settings');
        ...
        if ($LRP_LANGUAGE !== $settings['default-language']) {
            ...
            $lrp_search = $lrp->get_component('search');
            $include = $lrp_search->get_post_ids_containing_search_term($search_query, null);
 ...
 */
 add_action('init', 'lrp_mylisting_hook_exclude_string' );
 function lrp_mylisting_hook_exclude_string(){
     if( class_exists( 'MyListing\\App' ) ){
         add_filter('gettext_with_context', 'lrp_mylisting_exclude_string', 101, 4 );
     }
 }

 function lrp_mylisting_exclude_string( $translation, $text, $context, $domain ){

     if( $domain == 'my-listing' && $text == 'my-listings' )
         $translation = LRP_Translation_Manager::strip_gettext_tags( $translation );
 ...
 */
add_filter('googlesitekit_canonical_home_url', 'lrp_googlesitekit_compatibility_home_url' );
function lrp_googlesitekit_compatibility_home_url( $url ) {
    $lrp = LRP_Lingua_Press::get_lrp_instance();
    $url_converter = $lrp->get_component('url_converter');
 ...
 * usage.
 */
add_filter('lrp_get_existing_translations', 'lrp_wpengine_query_limit_check', 10, 3 );
function lrp_wpengine_query_limit_check($dictionary, $prepared_query, $strings_array){
    if ( function_exists('is_wpe') && ( !defined ('WPE_GOVERNOR') || ( defined ('WPE_GOVERNOR') && WPE_GOVERNOR != false ) ) && strlen($prepared_query) >= 16000 ){
        ...
        $lrp_query->maybe_record_automatic_translation_error(array( 'details' => esc_html__("Detected long query limitation on WPEngine hosting. Some large pages may appear untranslated. You can remove limitation by adding the following to your site’s wp-config.php: define( 'WPE_GOVERNOR', false ); ", 'linguapress')), true );
        return false;
    }else{
        return $dictionary;
    }
...
if ( class_exists('WeDevs_Dokan')) {
    add_filter( 'lrp_skip_gettext_processing', 'lrp_exclude_dokan_date_strings', 20, 4 );
}
function lrp_exclude_dokan_date_strings($return, $translation, $text, $domain) {
...
function lrp_add_language_to_pms_wppb_restriction_redirect_url( $redirect_url ){
    ...
    return $url_converter->get_url_for_language( $LRP_LANGUAGE, $redirect_url, '' );

}

if( defined( 'PMS_VERSION' ) )
    add_filter( 'pms_restricted_post_redirect_url', 'lrp_add_language_to_pms_wppb_restriction_redirect_url' );

if( function_exists( 'wppb_plugin_init' ) )
    add_filter( 'wppb_restricted_post_redirect_url', 'lrp_add_language_to_pms_wppb_restriction_redirect_url' );

/**
 * Compatibility with wp-Typography
 * The $filters array is set to empty, so it does not affect the strings anymore in the function lrp_remove_filters_wp_typography.
 * Then it is reset with a higher priority by calling the function process() inside the lrp_add_filters_wp_typography function.
 */

if(class_exists('WP_Typography')) {
    add_action('plugins_loaded', 'lrp_wp_typography');
}

function lrp_wp_typography(){
    global $LRP_LANGUAGE;
    $lrp = LRP_Lingua_Press::get_lrp_instance();
    $lrp_settings = $lrp->get_component('settings');
    $settings = $lrp_settings->get_settings();
    ...
    if ($LRP_LANGUAGE !== $settings['default-language']) {
        add_filter( 'typo_content_filters', 'lrp_remove_filters_wp_typography' );
        add_filter( 'lrp_translated_html', 'lrp_add_filters_wp_typography', 100000, 1 );
        add_filter('run_wptexturize', '__return_null', 11);
    }
}

function lrp_remove_filters_wp_typography($filters){
    $filters = [];
...
function lrp_add_filters_wp_typography($final_html){
                ...
                foreach ($callstack_function["object"]->{"callbacks"}[10] as $key=>$value){
                    if(strpos($key, 'actionScheduler')){
                        if(array_key_exists('breadcrumbs_archiveFormat',  $callstack_function["object"]->{"callbacks"}[10][ $key ]["function"][0]->{"options"}->{"localized"}  )) {
                            add_action( 'lrp_before_running_hooks', 'lrp_AIOSEO_remove_gettext_hooks', 10, 1 );
                        }
                    }
                }
            }
...
function lrp_AIOSEO_remove_gettext_hooks($lrp_loader){
    $lrp                = LRP_Lingua_Press::get_lrp_instance();
    $translation_render = $lrp->get_component( 'translation_render' );
    $lrp_loader->remove_hook( 'the_title', 'wrap_with_post_id', $translation_render );
}
 ...
 */
add_filter( 'lrp_needed_language', 'lrp_page_builders_compatibility_with_subdirectory_for_default_language', 10, 4 );
function lrp_page_builders_compatibility_with_subdirectory_for_default_language( $needed_language, $lang_from_url, $settings, $lrp) {
 ...
 */
add_filter( 'lrp_stop_translating_page', 'lrp_give_wp_compatibility', 10, 2 );
function lrp_give_wp_compatibility( $bool, $output ){
...
/*
 * Divi is filtering the locale which is in turn accessed on every gettext call by LinguaPress. Together these two things slow down the site to 20+ seconds
 ...
 */
add_filter('locale', 'lrp_remove_divi_locale_filter', 999999);
function lrp_remove_divi_locale_filter($lang){
...
**/
function lrp_et_divi_maybe_change_frontend_locale( $locale ) {
...
add_filter( 'locale', 'lrp_et_divi_maybe_change_frontend_locale' );

/*
 * Register old advanced settings if they are checked
 */
add_action('admin_init', 'lrp_register_old_advanced_settings');

function lrp_register_old_advanced_settings( $bool )
{

    $option = get_option('lrp_advanced_settings', true);
    if (isset($option['fix_broken_html']) && $option['fix_broken_html'] === 'yes') {

        add_filter('lrp_register_advanced_settings', 'lrp_register_fix_broken_html', 50);
    }
}

add_filter('lrp_ald_popup_options_array', 'lrp_keep_no_popup_setting_for_redirect_directly', 10, 1);

function lrp_keep_no_popup_setting_for_redirect_directly($array_popup_options){
    $option_ald = get_option('lrp_ald_settings', true);
    if (isset($option_ald['popup_option']) && $option_ald['popup_option'] !== 'no_popup' && version_compare(LRP_IN_ALD_PLUGIN_VERSION, '1.1', '>=') ) {
...
/**
 * Prevent lrp-sortable-languages.js script from running
 *
 * We have merged the code from lrp-sortable-languages.js in lrp-back-end-script.js in TP ver. 2.5.3 but we still need lrp-sortable-languages for backwards compatibility
 * If the version of LinguaPress is at least 2.5.3, prevent the lrp-sortable-languages.js script from running
 *
 */
add_action( 'lrp_before_running_hooks', 'lrpc_prevent_sortable_script_from_loading' );
function lrpc_prevent_sortable_script_from_loading( $lrp_loader ){
    if ( version_compare(LRP_PLUGIN_VERSION, '2.5.4', '>=' ) ) {
        $lrp_loader->remove_hook( 'admin_enqueue_scripts', 'enqueue_sortable_language_script' );
    }
}

add_filter('lrp_advanced_tab_add_element', 'lrp_compatibility_for_adl_127_version', 20);
function lrp_compatibility_for_adl_127_version( $settings ){
...
add_action( 'before_woocommerce_init', function() {
    if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
        \Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', LRP_PLUGIN_DIR . 'index.php', true );
    }
} );
 ...
 */
add_filter( 'rank_math/analytics/get_translated_objects', 'lrp_rank_math_get_translated_items', 10, 1 );
function lrp_rank_math_get_translated_items( $post_id ) {
    if ( ! class_exists( 'LRP_Lingua_Press' ) || !function_exists('lrp_translate')) {
        return $post_id;
    }

    $lrp                = LRP_Lingua_Press::get_lrp_instance();
    $url_converter      = $lrp->get_component( 'url_converter' );
    $settings_component = $lrp->get_component( 'settings' );
    $lrp_settings       = $settings_component->get_settings();

    // Needed because adding language slug in urls is not performed by default in admin area.
    add_filter( 'lrp_add_language_to_home_url_check_for_admin', '__return_false' );

    $permalink = get_permalink( $post_id );

    $translated_items = [];

    $languages = $lrp_settings['publish-languages'];
    foreach ( $languages as $language ) {
        ...
        if( $language != $lrp_settings['default-language'] ){
            $title = lrp_translate( $title, $language, false );
        }
    ...
    remove_filter( 'lrp_add_language_to_home_url_check_for_admin', '__return_false' );
...
if (class_exists("Tribe__Events__Adjacent_Events")) {
    add_filter('lrp_filter_post_type_base_slugs_from_automatic_translation', 'lrp_stop_automatic_translation_for_certain_post_type_base_slugs', 10, 2);
}

function lrp_stop_automatic_translation_for_certain_post_type_base_slugs( $bool, $post_type_base_slug_to_avoid ) {
...
if (class_exists("duplicate_page")) {
    ...
    add_action('save_post', 'lrp_add_hook_for_delete', 10, 1);
}

function lrp_add_hook_for_delete( $post_id )
{
    global $lrp_post_id_for_deleting_duplicate_posts_slugs_from_db;

    if (did_action('admin_action_dt_duplicate_post_as_draft')) {
        $lrp_post_id_for_deleting_duplicate_posts_slugs_from_db = $post_id;
        add_action('shutdown', 'lrp_delete_slug_translation_from_duplicated_pages' );
    }
}
function lrp_delete_slug_translation_from_duplicated_pages() {
    global $lrp_post_id_for_deleting_duplicate_posts_slugs_from_db;
    global $wpdb;

    $sql = $wpdb->prepare("DELETE FROM $wpdb->postmeta WHERE post_id = %d AND (meta_key LIKE %s OR meta_key LIKE %s);", $lrp_post_id_for_deleting_duplicate_posts_slugs_from_db, '%'. $wpdb->esc_like('lrp_automatically_translated_slug') .'%', '%'. $wpdb->esc_like('lrp_translated_slug') .'%');

    $wpdb->query( $sql );
    
    unset( $GLOBALS['lrp_post_id_for_deleting_duplicate_posts_slugs_from_db'] );
...
if ( class_exists( 'QueryMonitor' ) ) {
    add_filter( 'lrp_skip_gettext_processing', 'lrp_exclude_query_monitor_strings', 10, 4 );
    add_filter( 'lrp_no_translate_selectors', 'lrp_exclude_query_monitor_selector' );
    add_filter( 'lrp_skip_selectors_from_dynamic_translation', 'lrp_exclude_query_monitor_selector' );
}
function lrp_exclude_query_monitor_strings( $bool, $translation, $text, $domain ){
 ...
 */
function lrp_exclude_query_monitor_selector( $skip_selectors ) {
...
/**
 * Compatibility with Complianz plugin blocking lrp_data script
 */
// Whitelisting inline script for Complianz
add_filter ( 'cmplz_service_category', 'lrp_cmplz_whitelist_script', 10 , 3 );
function lrp_cmplz_whitelist_script( $category, $total_match, $found ){
    if ( $found && false !== strpos( $total_match, 'lrp-dynamic-translator-js-extra' ) ) {
 ...
 */
add_filter('lrp_stop_translating_page', 'lrp_do_not_translate_fluent_form_submit', 1000000, 2);
function lrp_do_not_translate_fluent_form_submit($translate, $output){
 ...
 */
add_filter('lrp_stop_translating_page', 'lrp_do_not_translate_woo_bookings_cost_calculator', 10, 2);
function lrp_do_not_translate_woo_bookings_cost_calculator($translate, $output){
...
add_action('init', 'lrp_woo_bookings_gettext_filter');
function lrp_woo_bookings_gettext_filter(){
    if ( isset( $_POST['action'] )  && $_POST['action'] == 'wc_bookings_calculate_costs'){
        add_filter('gettext', 'lrp_woo_bookings_exclude_gettext_strings', 1000, 3 );
    }
}

function lrp_woo_bookings_exclude_gettext_strings($translation, $text, $domain){
    if ( isset( $_POST['action'] )  && $_POST['action'] == 'wc_bookings_calculate_costs' ){
        return LRP_Translation_Manager::strip_gettext_tags( $translation );
    }
    return $translation;
 ...
 */
add_filter( 'the_excerpt_rss', 'lrp_translate_the_excerpt_rss', 10, 1);
function lrp_translate_the_excerpt_rss( $output ){
    $lrp = LRP_Lingua_Press::get_lrp_instance();
    $translation_render = $lrp->get_component( 'translation_render' );
    return $translation_render->translate_page($output);
};

add_filter( 'the_content_feed', 'lrp_translate_the_content_feed', 10, 2);
function lrp_translate_the_content_feed( $content, $feed_type ){
    $lrp = LRP_Lingua_Press::get_lrp_instance();
    $translation_render = $lrp->get_component( 'translation_render' );
...
add_filter('lrp_stop_translating_page', 'lrp_block_wpjb_pro_pdf_translation', 10, 2);
 ...
 * related to PDF files. If so, it prevents these PDF files from being translated by LinguaPress.
 *
 ...
 */
function lrp_block_wpjb_pro_pdf_translation( $bool, $output ) {
...
if ( function_exists( 'brikk_utilities_load_textdomain' ) ) {
    add_filter( 'lrp_skip_form_action', 'lrp_exclude_brikk_theme_form_action', 10, 2 );
}
function lrp_exclude_brikk_theme_form_action( $skip_this_action, $form_action ) {
 ...
 */
add_filter('lrp_stop_translating_page', 'lrp_do_not_translate_service_worker_pages', 10, 2);
function lrp_do_not_translate_service_worker_pages($translate, $output){
...
if ( class_exists( 'WooCommerce' ) ){
    add_action('plugins_loaded', 'lrp_check_if_woo_language_po_file_exists');
}

function lrp_check_if_woo_language_po_file_exists() {
    ...
    if ( $settings['default-language'] != 'en_US' && !file_exists(WP_LANG_DIR . "/plugins/woocommerce-{$settings['default-language']}.po")){
        lrp_download_woo_po_file_for_default_language( $settings['default-language'] );
    }
}

function lrp_download_woo_po_file_for_default_language( $default_language ){

    $path_for_po_file_in_the_requested_language = lrp_get_translation_woo_po_files_url( $default_language );
...
function lrp_get_translation_woo_po_files_url( $default_language ) {
...
/**
 * Hooked to lrp_get_url_for_language, used in case the pro version of TP was not updated or legacy SEO Pack is in use
 *
 * @param $new_url
 * @param $url
 * @param $language
 ...
 */
function lrp_get_url_for_language_backwards_compatibility( $new_url, $url, $language ){
    global $lrp_current_url_term_slug, $lrp_current_url_taxonomy;

    $lrp = LRP_Lingua_Press::get_lrp_instance();
    $lrp_settings = $lrp->get_component( 'settings' );
    $settings = $lrp_settings->get_settings();

    $upgrade = new LRP_Upgrade( $settings );

    if ( $upgrade->is_seo_pack_minimum_version_met() && ( !isset( $settings['lrp_advanced_settings']['load_legacy_seo_pack'] ) || $settings['lrp_advanced_settings']['load_legacy_seo_pack'] === 'no' ) ) return $new_url; // Abort -- New system can be used, process URL via get_slug_translated_url_for_language
    ...
    $lrp_language_copy = $LRP_LANGUAGE;

    $url_obj          = lrp_cache_get('url_obj_' . hash('md4', $url), 'lrp');
    $abs_home_url_obj = lrp_cache_get('url_obj_' . hash('md4',  $url_converter->get_abs_home() ), 'lrp');

    $possible_post_id = lrp_cache_get( 'possible_post_id_'. hash('md4', $url ), 'lrp' );

    if ( $possible_post_id ){
        $post_id = $possible_post_id;
        lrp_bulk_debug($debug, array('url' => $url, 'found post id' => $post_id, 'for language' => $LRP_LANGUAGE));
    }

    else {
        $post_id = url_to_postid( $url );
        wp_cache_set( 'possible_post_id_' . hash('md4', $url ), $post_id, 'lrp' );
        if ( $post_id ) { lrp_bulk_debug($debug, array('url' => $url, 'found post id' => $post_id, 'for default language' => $LRP_LANGUAGE)); }
        ...
        if ( $post_id == 0 ) {
            ...
            */
            ...
            add_filter('lrp_keep_permalinks_unchanged', '__return_true' );
            ...
            */
            ...
            wp_cache_set( 'possible_post_id_' . hash('md4', $possible_url ), $post_id, 'lrp' );
            if($post_id){ lrp_bulk_debug($debug, array('url' => $url, 'found post id' => $post_id, 'for default language' => $LRP_LANGUAGE)); }

            remove_filter('lrp_keep_permalinks_unchanged', '__return_true' );
            $LRP_LANGUAGE = $lrp_language_copy;
        }
    }

    $LRP_LANGUAGE = $url_converter->get_lang_from_url_string( $url );

    if ($LRP_LANGUAGE == null){
        $LRP_LANGUAGE = $settings['default-language'];
    }
    ...
    if( $post_id ){
         ...
         *
         * WE ARE NOT USING \LinguaPress\Uri
        ...
        lrp_bulk_debug($debug, array('url' => $url, 'new url' => $new_url, 'found post id' => $post_id, 'url type' => 'based on permalink', 'for language' => $LRP_LANGUAGE));
        $LRP_LANGUAGE = $lrp_language_copy;
    ...
    if( isset( $lrp_current_url_term_slug ) && isset($lrp_current_url_taxonomy) && $new_url_has_been_determined === false ){
        // check here if it is a term link
        $current_term_link = get_term_link( $lrp_current_url_term_slug, $lrp_current_url_taxonomy);

        if (!is_wp_error($current_term_link)){
            $LRP_LANGUAGE = $language;
            $check_term_link = get_term_link($lrp_current_url_term_slug, $lrp_current_url_taxonomy);
    ...
    if ($new_url_has_been_determined === false){
        ...
        if ($url_converter->get_lang_from_url_string($url) === null) {
            ...
            lrp_bulk_debug($debug, array('url' => $url, 'new url' => $new_url, 'lang' => $language, 'url type' => 'custom url without language parameter'));
        } else {
            ...
            lrp_bulk_debug($debug, array('url' => $url, 'new url' => $new_url, 'lang' => $language, 'url type' => 'custom url with language', 'abs home path' => $abs_home_url_obj->getPath()));
...
add_filter('lrp_get_url_for_language', 'lrp_get_url_for_language_backwards_compatibility', 10, 3 );
 ...
 */
function lrp_compatibility_profile_builder_redirect( $redirect_url ) {
    $lrp_instance = LRP_Lingua_Press::get_lrp_instance();

    global $LRP_LANGUAGE;

    $url_converter  = $lrp_instance->get_component( 'url_converter' );

    return $url_converter->get_url_for_language( $LRP_LANGUAGE, $redirect_url, '' );
}

add_filter( 'wppb_register_redirect', 'lrp_compatibility_profile_builder_redirect' );
add_filter( 'wppb_edit_profile_redirect', 'lrp_compatibility_profile_builder_redirect' );
 ...
 */
add_filter('lrp_curpageurl', 'lrp_wpbackery_compatibility_remove_params_curpageurl');
add_filter('lrp_get_url_for_language', 'lrp_wpbackery_compatibility_remove_params', 10, 3);

function lrp_wpbackery_compatibility_remove_params($url, $language, $args) {
    return lrp_wpbackery_compatibility_strip_params($url);
}

function lrp_wpbackery_compatibility_remove_params_curpageurl($url) {
    return lrp_wpbackery_compatibility_strip_params($url);
}

function lrp_wpbackery_compatibility_strip_params($url) {
    ...
    // Use LinguaPress URI class for proper URL handling
    $uri = new \LinguaPress\Uri($url);
