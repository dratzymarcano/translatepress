...
class LRP_IN_Deepl_Machine_Translator extends LRP_Machine_Translator {
    ...
    public function send_request( $source_language, $language_code, $strings_array, $formality = "default" ){
        ...
        /*
         * Adds a filter so the user can add their glossary IDs to the requests
         */
        ...
        if ( ! empty( $glossary_language_pairs_user_input ) ) {
            ...
            $source_language_without_region = strtolower( strtok( $source_language, '_' ) );
            $target_language_without_region = strtolower( strtok( $language_code, '_' ) );
            ...
            if ( $glossary_id ) {
                ...
                $translation_request = '&source_lang=' . $source_language_without_region;
                $translation_request .= '&target_lang=' . $target_language_without_region;
                $translation_request .= '&split_sentences=1';
                $translation_request .= '&glossary_id=' . $glossary_id;
...
/*
 * IMPORTANT: As of March 2025, DeepL is no longer allowing
 * authentification of a request to any endpoint by sending an API key in a query parameter.
 * Instead, send your API key in an HTTP header named "Authorization"

   Other changes:  GET requests or query parameters will not be allowed anymore, "/translate" will accept
   only POST requests with data included in the request body
